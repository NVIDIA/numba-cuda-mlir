...
For pre-Blackwell GPUs (< sm_100), numba-cuda-mlir uses the mlir-llvm70 translator
...
To dump Numba IR and MLIR to stderr before MLIR-to-NVVM pipeline, enable `dump` in `@numba_cuda_mlir.jit()` decorator options. i.e., use `@numba_cuda_mlir.jit(dump=True)`.
To print full list of available debug options, enable `help` in `@numba_cuda_mlir.jit()` decorator options. i.e., use `@numba_cuda_mlir.jit(help=True)`.
