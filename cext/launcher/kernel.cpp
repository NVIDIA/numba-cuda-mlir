...
static uint16_t double_to_f16_bits(double d) {
    ...
    if (exp == 128) {
        ...
        return static_cast<uint16_t>(
            sign | (static_cast<uint16_t>(exp + 15) << 10) | static_cast<uint16_t>(mant >> 13));
    } else if (exp >= -24) {
        mant |= 0x800000;
        return static_cast<uint16_t>(sign | static_cast<uint16_t>(mant >> (-exp - 14 + 23)));
    }
    return sign;
}
...
#define INIT_STRING_CONSTANT(ident) \
    g_##ident##_pyunicode = PyUnicode_InternFromString(#ident); \
    if (!g_##ident##_pyunicode) return ErrorRaised;
