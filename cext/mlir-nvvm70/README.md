...
MLIR-to-NVVM70 translator for cuSIMT. Walks `gpu.module` ops (LLVM dialect)
...
- **`NVVM70IRBuilder`** — wraps the old `libLLVM.so` C API (~80 function
  pointers). Builds LLVM 7 IR in memory.
- **`LibNVVMCompiler`** — wraps `libnvvm.so` to compile NVVM bitcode to PTX.
- **`MLIRToNVVM70`** — walks a `gpu.module` and translates each MLIR op to the
  corresponding `NVVM70IRBuilder` call.
- **`#nvvm70.target`** — TableGen-defined target attribute implementing
  `gpu::TargetAttrInterface`, so GPU modules can be compiled via the standard
  `gpu-module-to-binary` pass.
- **`nvvm70-translate`** — standalone CLI tool (used by LIT tests).
...
This produces `libMLIRToNVVM70.so` and places it next to `_cext.so`.
...
export NVVM70_LIBNVVM=/path/to/cuda/nvvm/lib64/libnvvm.so
export CUDA_HOME=/path/to/cuda

# Single-config run:
ninja -C build check-nvvm70

# Multi-CUDA × multi-SM matrix:
ninja -C build check-nvvm70-matrix
```
...
The `#nvvm70.target` attribute supports:
...
# Dump PTX to stderr
nvvm70-translate input.mlir --dump-ptx

# Dump intermediate LLVM IR to stderr
nvvm70-translate input.mlir --dump-llvm

# Produce host LLVM IR with embedded PTX
nvvm70-translate input.mlir --mlir-to-llvm-ir -o output.ll
```

Library paths are resolved from (in priority order):
1. `#nvvm70.target<libllvm = "...", libnvvm = "...">` attribute
2. `LIBLLVM7` / `NVVM70_LIBNVVM` environment variables
