 ...
 */
//===- NVVM70IRBuilder.h - Old LLVM C API wrapper ----------------*- C++ -*-===//
//
...
#ifndef NVVM70_NVVM70IRBUILDER_H
#define NVVM70_NVVM70IRBUILDER_H

#include "nvvm70/CAPILoader.h"
...
//===----------------------------------------------------------------------===//
// NVVM70IRBuilder — builds LLVM 7 IR via dlopen'd libLLVM-7.so
//===----------------------------------------------------------------------===//
class NVVM70IRBuilder {
public:
  static llvm::Expected<std::unique_ptr<NVVM70IRBuilder>>
  create(llvm::StringRef libLLVMPath);

  ~NVVM70IRBuilder();
  NVVM70IRBuilder(const NVVM70IRBuilder &) = delete;
...
#endif // NVVM70_NVVM70IRBUILDER_H
