 ...
 */
//===- NVVM70Target.h - MLIR → old LLVM IR → PTX via C API -------*- C++ -*-===//
//
// Walks a gpu.module containing LLVM dialect ops, builds LLVM 7 IR
// through the old LLVM C API (via NVVM70IRBuilder), and compiles to PTX through
// libnvvm (via LibNVVMCompiler).
//
//===----------------------------------------------------------------------===//

#ifndef NVVM70_NVVM70TARGET_H
#define NVVM70_NVVM70TARGET_H

#include "nvvm70/LibNVVMCompiler.h"
...
class MLIRToNVVM70 {
public:
  MLIRToNVVM70(NVVM70IRBuilder &builder) : b(builder) {}
