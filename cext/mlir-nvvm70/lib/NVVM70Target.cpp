 ...
 */
//===- NVVM70Target.cpp - MLIR → old LLVM IR → PTX via C API -----*- C++ -*-===//
// 
//===----------------------------------------------------------------------===//

#include "nvvm70/NVVM70Target.h"
...
#define DEBUG_TYPE "nvvm70-target"
...
llvm::Expected<std::string> nvvm70::translateToNVVMIR(gpu::GPUModuleOp gpuMod,
                                                     const NVVM70Options &opts) {
  auto builderOrErr = NVVM70IRBuilder::create(opts.libLLVMPath);
  if (!builderOrErr)
  ...
  MLIRToNVVM70 translator(*builder);
...
llvm::Expected<std::string> nvvm70::translateToPTX(gpu::GPUModuleOp gpuMod,
                                                  const NVVM70Options &opts) {
  auto builderOrErr = NVVM70IRBuilder::create(opts.libLLVMPath);
  if (!builderOrErr)
    return builderOrErr.takeError();
...
static uint64_t toUInt64(const llvm::APInt &val) {
  if (val.getActiveBits() > 64)
    llvm::report_fatal_error("nvvm70: integer constant exceeds 64 bits",
        /*GenCrashDiag=*/false);
...
static LLVMValueRef constBF16(NVVM70IRBuilder &b, const llvm::APFloat &v) {
...
LLVMValueRef MLIRToNVVM70::bf16ToF32(LLVMValueRef i16Val) {
...
LLVMValueRef MLIRToNVVM70::f32ToBf16(LLVMValueRef f32Val) {
...
LLVMTypeRef MLIRToNVVM70::convertType(Type ty) {
...
LLVMValueRef MLIRToNVVM70::lookupValue(Value v) {
  auto it = valueMap.find(v);
  if (it != valueMap.end())
    return it->second;
  std::string msg;
  llvm::raw_string_ostream os(msg);
  os << "nvvm70: unmapped MLIR value: " << v
...
MLIRToNVVM70::extractFileLineCol(Location loc) {
...
LLVMMetadataRef MLIRToNVVM70::getOrCreateDIFile(llvm::StringRef filename) {
...
void MLIRToNVVM70::setDebugLocFromOp(Operation *op) {
  if (!diCompileUnit)
    return;
...
llvm::Error MLIRToNVVM70::translate(gpu::GPUModuleOp gpuMod, int debugLevel) {
  ...
  if (debugLevel > 0) {
    ...
    b.initDebugInfo();
    constexpr llvm::StringLiteral moduleName("nvvm70_module");
...
llvm::Error MLIRToNVVM70::translateGlobalOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateFuncOp(Operation *op) {
  auto funcOp = cast<LLVM::LLVMFuncOp>(op);
  ...
  if (diCompileUnit) {
    ...
    LLVMMetadataRef diFile =
        filename.empty() ? getOrCreateDIFile("nvvm70_module")
...
llvm::Error MLIRToNVVM70::translateBlock(Block &block) {
...
llvm::Error MLIRToNVVM70::translatePhiOps(Block &block) {
  if (block.isEntryBlock())
    return llvm::Error::success();
...
llvm::Error MLIRToNVVM70::translateOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateReturnOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateBrOp(Operation *op) {
  auto brOp = cast<LLVM::BrOp>(op);
  b.buildBr(blockMap[brOp.getDest()]);
  return llvm::Error::success();
}

llvm::Error MLIRToNVVM70::translateCondBrOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateSwitchOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateCallOp(Operation *op) {
  ...
  if (!callee)
    return llvm::createStringError(llvm::inconvertibleErrorCode(),
                                   "unresolved callee in llvm.call");
...
llvm::Error MLIRToNVVM70::translateConstantOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateAddressOfOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateArithOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateICmpOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateFCmpOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateLoadOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateStoreOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateAllocaOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateGEPOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateCastOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateSelectOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateExtractValueOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateInsertValueOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateExtractElementOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateInsertElementOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateInlineAsmOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateFrexpOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateLdexpOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateSimpleIntIntrinsic(Operation *op,
...
llvm::Error MLIRToNVVM70::translateUnaryFloatIntrinsic(Operation *op,
                                                       llvm::StringRef intrBase) {
  Value in = op->getOperand(0);
  Value res = op->getResult(0);
  Type valTy = in.getType();
...
llvm::Error MLIRToNVVM70::translateBinaryIntIntrinsic(Operation *op,
...
llvm::Error MLIRToNVVM70::translateAbsOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateBinaryFloatIntrinsic(Operation *op,
                                                        llvm::StringRef intrBase) {
...
llvm::Error MLIRToNVVM70::translateMinimumMaximumOp(Operation *op,
...
llvm::Error MLIRToNVVM70::translateMemsetOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateMemcpyOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateLifetimeOp(Operation *op, bool isStart) {
...
llvm::Error MLIRToNVVM70::translateClusterArriveOp(Operation *op,
                                                   bool isRelaxed) {
  bool aligned = false;
...
llvm::Error MLIRToNVVM70::translateClusterWaitOp(Operation *op) {
  bool aligned = false;
...
llvm::Error MLIRToNVVM70::translateBarrierOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateCttzCtlzOp(Operation *op,
                                              llvm::StringRef intrBase,
                                              Value in, Value res,
                                              bool isZeroPoison) {
  Type valTy = in.getType();
...
llvm::Error MLIRToNVVM70::translateFloatAtomicCASLoop(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateAtomicRMWOp(Operation *op) {
  auto rmwOp = cast<LLVM::AtomicRMWOp>(op);
...
llvm::Error MLIRToNVVM70::translateAtomicCmpXchgOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateSregOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateBarrier0Op(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateFmaOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateElectSyncOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateMatchSyncOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateNanosleepOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateReduxSyncOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateMembarOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateSyncWarpOp(Operation *op) {
  LLVMTypeRef paramTy = b.i32Ty();
  LLVMTypeRef fnTy = b.funcTy(b.voidTy(), &paramTy, 1, false);
...
llvm::Error MLIRToNVVM70::translateVoteSyncOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateShflOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateNVVMOp(Operation *op) {
  return llvm::TypeSwitch<Operation *, llvm::Error>(op)
...
LLVMMetadataRef MLIRToNVVM70::getOrCreateDIType(LLVM::DITypeAttr typeAttr) {
  if (!typeAttr) {
...
llvm::Error MLIRToNVVM70::translateDbgDeclareOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::translateDbgValueOp(Operation *op) {
...
llvm::Error MLIRToNVVM70::emitDbgIntrinsic(Operation *op, LLVMValueRef val,
  ...
  LLVMMetadataRef diFile = varInfo.getFile()
      ...
      : getOrCreateDIFile("nvvm70_module");
...
void MLIRToNVVM70::emitKernelMetadata(LLVMValueRef fn, Operation *funcOp) {
