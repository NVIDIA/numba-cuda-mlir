 ...
 */
//===- Target.cpp - NVVM70 target compilation --------------------*- C++ -*-===//
//
// Implements the TargetAttrInterface for #nvvm70.target.  serializeToObject()
// bypasses the normal ModuleToObject pipeline and instead:
//   1. Walks the MLIR gpu.module directly with MLIRToNVVM70
...
namespace {
class NVVM70TargetAttrImpl
    : public gpu::TargetAttrInterface::FallbackModel<NVVM70TargetAttrImpl> {
public:
...
void ::nvvm70::registerNVVM70TargetInterfaceExternalModels(
    DialectRegistry &registry) {
  registry.addExtension(
      +[](MLIRContext *ctx, mlir::nvvm70::NVVM70Dialect *dialect) {
        NVVM70TargetAttr::attachInterface<NVVM70TargetAttrImpl>(*ctx);
      });
}

void ::nvvm70::registerNVVM70TargetInterfaceExternalModels(MLIRContext &context) {
  DialectRegistry registry;
...
static std::string resolveLibLLVMPath(NVVM70TargetAttr target) {
...
static std::string resolveLibNVVMPath(NVVM70TargetAttr target,
                                      StringRef toolkitPath) {
  StringRef path = target.getLibnvvm();
  if (!path.empty())
    return path.str();
  if (const char *env = std::getenv("NVVM70_LIBNVVM"))
    return env;
...
std::optional<SmallVector<char, 0>> NVVM70TargetAttrImpl::serializeToObject(
  ...
  auto target = cast<NVVM70TargetAttr>(attribute);

  // Resolve paths.
  std::string libllvmPath = resolveLibLLVMPath(target);
  if (libllvmPath.empty()) {
    module->emitError()
        << "NVVM70: libLLVM path not set. Use #nvvm70.target<libllvm=...>, "
  ...
  if (libnvvmPath.empty()) {
    module->emitError()
        << "NVVM70: libnvvm path not set. Use #nvvm70.target<libnvvm=...>, "
           "set NVVM70_LIBNVVM env, or set CUDA_ROOT.";
    return std::nullopt;
  }

  // Build NVVM70Options.
  ::nvvm70::NVVM70Options opts;
  ...
  constexpr StringLiteral kIRDumpFlag("--nvvm70-ir-dump=");
  constexpr StringLiteral kIRStderrFlag("--nvvm70-ir-stderr");
  constexpr StringLiteral kChipFlag("--nvvm70-chip=");
  bool dumpIRToFile = cmdOpts.contains(kIRDumpFlag);
  bool dumpIRToStderr = cmdOpts.contains(kIRStderrFlag);

  // --chip override from cmdOptions or NVVM70_CHIP env var.
  if (cmdOpts.contains(kChipFlag)) {
    StringRef tail =
        cmdOpts.substr(cmdOpts.find(kChipFlag) + kChipFlag.size());
    opts.chip = tail.split(' ').first.str();
  } else if (const char *envChip = std::getenv("NVVM70_CHIP")) {
    if (envChip[0] != '\0')
      opts.chip = envChip;
  }
  ...
  // Validate SM range: supports sm_75 through sm_9x.
  {
    ...
    if (chip.starts_with("sm_") &&
      ...
      if (smVersion < 75) {
        module->emitError()
            << "NVVM70 does not support " << opts.chip
  ...
  if (dumpIRToFile || dumpIRToStderr) {
    auto irOrErr = ::nvvm70::translateToNVVMIR(gpuMod, opts);
    if (!irOrErr) {
      module->emitError() << "NVVM70 IR generation failed: "
  ...
  auto ptxOrErr = ::nvvm70::translateToPTX(gpuMod, opts);
  if (!ptxOrErr) {
    module->emitError() << "NVVM70 translation failed: "
...
Attribute NVVM70TargetAttrImpl::createObject(
    Attribute attribute, Operation *module,
    const gpu::SerializedObject &object,
    const gpu::TargetOptions &options) const {
