...
# Run the nvvm70 test suite across a matrix of CUDA versions × SM architectures.
#
# Usage:
...
BUILD_DIR="${NVVM70_BUILD_DIR:-$PROJECT_DIR/build}"
...
for cuda_ver in "${CUDAS[@]}"; do
  ...
  for sm in "${SMS[@]}"; do
    ...
    if NVVM70_LIBNVVM="$LIBNVVM" NVVM70_CHIP="$sm" \
