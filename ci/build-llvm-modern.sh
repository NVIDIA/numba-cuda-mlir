...
# Build modern LLVM + MLIR from source for cuSIMT.
...
# Configure
# MLIR_PYTHON_PACKAGE_PREFIX bakes "cusimt._mlir." into all compiled .so
#   files, so bindings expect cusimt._mlir.ir (not mlir.ir) at runtime.
...
cmake_args=(
    ...
    -DCMAKE_CXX_FLAGS="-DMLIR_PYTHON_PACKAGE_PREFIX=cusimt._mlir."
    -DMLIR_BINDINGS_PYTHON_INSTALL_PREFIX="python_packages/cusimt_mlir/cusimt/_mlir"
    -DMLIR_BINDINGS_PYTHON_NB_DOMAIN=cusimt
...
ls -lh "${LLVM_INSTALL}/python_packages/cusimt_mlir/cusimt/_mlir/_mlir_libs/libMLIRPythonCAPI.so" 2>/dev/null || true
