...
- `python_packages/cusimt_mlir/cusimt/_mlir/` — MLIR Python bindings
  namespaced under `cusimt._mlir` (not top-level `mlir`)
...
The modern LLVM build uses several flags to produce cuSIMT-compatible
...
| `-DCMAKE_CXX_FLAGS="-DMLIR_PYTHON_PACKAGE_PREFIX=cusimt._mlir."` | Namespace bindings under `cusimt._mlir` |
| `-DMLIR_BINDINGS_PYTHON_INSTALL_PREFIX=python_packages/cusimt_mlir/cusimt/_mlir` | On-disk install path |
| `-DMLIR_BINDINGS_PYTHON_NB_DOMAIN=cusimt` | Isolate nanobind typeids |
| `-DCMAKE_PLATFORM_NO_VERSIONED_SONAME=ON` | Ship `lib*.so` without versioned symlinks (required for wheels) |

## Using the build

After building, install cuSIMT pointing at the artifacts:
