...
NUMBA_CUDA_MLIR_ROOT = Path(__file__).resolve().parent.parent.parent
...
def _default_env() -> dict:
    ...
    env.setdefault("NUMBA_CUDA_MLIR_ICE_FULL_TB", "1")
    return env
...
class VEnv:
    ...
    def install_project(self, extras: str = "") -> subprocess.CompletedProcess:
        """Install the current project (numba_cuda_mlir) in editable mode."""
        spec = f"-e .[{extras}]" if extras else "-e ."
        return self.run_pip("install", spec, cwd=NUMBA_CUDA_MLIR_ROOT)
...
def resolve_venv(args: argparse.Namespace, keep_venv_path: str, temp_prefix: str):
    if args.venv:
        ...
        venv = VEnv.create(NUMBA_CUDA_MLIR_ROOT / keep_venv_path)
        yield venv
...
def install_mlir(venv: VEnv) -> None:
    ...
    what libMLIRToLLVM70.so was compiled against.  Otherwise downloads
    pre-built MLIR Python packages from eudsl.
    """
...
def install_numba_cuda_mlir_editable(
    venv: VEnv, extras: str | None = None, numba_cuda_mlir_root: Path | None = None
) -> None:
    """Install numba_cuda_mlir into a venv.

    If a pre-built wheel is available in <numba_cuda_mlir_root>/dist/, install that
    ...
    root = numba_cuda_mlir_root or NUMBA_CUDA_MLIR_ROOT

    # Prefer a pre-built wheel if one exists — avoids rebuilding the
    # C extension + MLIR bindings in every test job.
    wheels = sorted((root / "dist").glob("numba_cuda_mlir-*.whl"))
    if wheels:
        wheel = wheels[-1]
        spec = f"{wheel}[{extras}]" if extras else str(wheel)
        print(f"Installing numba_cuda_mlir from pre-built wheel: {wheel.name}")
