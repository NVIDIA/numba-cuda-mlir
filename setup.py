...
def _find_mlir_python_capi() -> str | None:
    ...
    capi = sp / "cusimt" / "_mlir" / "_mlir_libs" / "libMLIRPythonCAPI.so"
    return str(capi) if capi.exists() else None


class BuildExtWithCmake(build_ext):
    def run(self):
        build_dir = os.getenv("CUSIMT_BUILD_DIR")
    ...
    def _stage_mlir_bindings(self):
        ...
        if not mlir_dir:
            return
        install_root = Path(mlir_dir).resolve().parent.parent.parent
        mlir_pkg = install_root / "python_packages" / "cusimt_mlir" / "cusimt" / "_mlir"
        if not mlir_pkg.exists():
            print(f"WARNING: MLIR Python bindings not found at {mlir_pkg}")
            return
        pkg = (
            Path(self.get_ext_fullpath("cusimt._cext")).parent
            if self.editable_mode
            else Path(self.build_lib) / "cusimt"
        )
        dest = pkg / "_mlir"
        if self.editable_mode:
            if dest.exists() or dest.is_symlink():
...
VERSION = os.getenv("CUSIMT_VERSION")
if VERSION is None:
    version_file = ROOT / "src" / "cusimt" / "VERSION"
    if not version_file.exists():
        raise RuntimeError(
            f"Version file {version_file} does not exist and CUSIMT_VERSION is not set in environment"
        )
...
setup(
    version=VERSION,
    ext_modules=[
        Extension("cusimt._cext", []),
        Extension("cusimt._typeconv", []),
        Extension("cusimt._mviewbuf", []),
        Extension("cusimt._helperlib", []),
    ],
    cmdclass=dict(
