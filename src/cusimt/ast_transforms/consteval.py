...
from cusimt.ast_transforms.common import get_function_context
from cusimt.ast_transforms.pipeline import ASTTransformPass, TransformContext
...
class TargetOptionsReplacer(ast.NodeTransformer):
    """Replace cuda.simt.current_target_options() calls with a context reference."""

    def visit_Call(self, node: ast.Call) -> ast.AST:
        node = self.generic_visit(node)
        # Match: cuda.simt.current_target_options() or current_target_options()
        if self._is_current_target_options_call(node):
            return ast.copy_location(ast.Name(id="__cusimt_target_options__", ctx=ast.Load()), node)
        return node
    ...
    def _is_current_target_options_call(self, node: ast.Call) -> bool:
        ...
        if isinstance(func, ast.Name) and func.id == "current_target_options":
            return True
        # cuda.simt.current_target_options() or simt.current_target_options()
...
class ConstevalTransformer(ast.NodeTransformer):
    ...
    6. Provides access to target options via cuda.simt.current_target_options()
    ...
    def context(self):
        ...
        ctx["__cusimt_target_options__"] = self.targetoptions
        return ctx
