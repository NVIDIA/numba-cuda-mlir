...
everything in numba.cuda that cusimt _also_ supports.
"""

import sys

# Base API comes from `numba.cuda` (which may be redirected to cuSIMT).
from cusimt.numba_cuda import *  # noqa: F403
...
def struct(*args, **kwargs):
    from cusimt.host import struct

    return struct(*args, **kwargs)


def union(*args, **kwargs):
    from cusimt.host import union
...
class _CurrentTargetOptionsMarker:
    ...
    def __repr__(self):
        return "cuda.simt.current_target_options()"
...
def current_target_options() -> dict:
    ...
    Example:
        ...
        def kernel(arr):
            chip = consteval(cuda.simt.current_target_options()['chip'])  # 'sm_90'
    """
