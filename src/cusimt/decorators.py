...
from cusimt.tools import get_gpu_compute_capability, format_arch
...
def _target_options_help(why: str | None = None):
    ...
    prelude = dedent(
        f"""
    {why}
    The available keyword options for cusimt.jit are:
...
def mlir_jit(func_or_sig=None, **kws):
    """
    cusimt JIT decorator. Use this function to decorate kernels and device functions.
    """
...
def stubgen(out: StringIO = sys.stdout):
    ...
    this_file = Path(__file__).relative_to(Path(cusimt.__path__[0]).parent)
