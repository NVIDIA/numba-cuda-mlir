...
from cusimt.numba_cuda.core.errors import ForceLiteralArg, TypingError
from cusimt._mlir.ir import Operation, Value, Block
...
def handle_lowering_error(lower, func_ir):
    ...
    Set CUSIMT_ICE_FULL_TB=1 to see the full traceback instead of a summary.
    """
    import os
    import tempfile
    import traceback
    ...
    full_tb_env = os.environ.get("CUSIMT_ICE_FULL_TB", "0").strip()

    log_fd, log_path = tempfile.mkstemp(suffix=".log", prefix="cusimt_error_")
    try:
    ...
    msg = (
        ...
        f"  {log_path}\n\n"
        f"NOTE: To quell this message and see the full traceback, set the environment variable CUSIMT_ICE_FULL_TB=1."
    )
    if full_tb_env == "1":
