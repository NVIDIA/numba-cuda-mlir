...
from cusimt._mlir.extras.meta import region_op
import operator
from cusimt.cuda.misc.special import literal_unroll, literally
from cusimt.numba_cuda.misc.special import literal_unroll as cuda_literal_unroll
from cusimt.errors import InternalCompilerError
...
from cusimt.descriptor import MLIRTargetContext
from cusimt.mlir_lowering import MLIRLower
from cusimt._mlir.dialects import arith, func, memref, gpu, nvvm, llvm
...
@lower(cusimt.cuda.selp, types.Integer, types.Integer, types.Integer)
@lower(cusimt.cuda.selp, types.Boolean, types.Integer, types.Integer)
@lower(cusimt.cuda.selp, types.Integer, types.Float, types.Float)
@lower(cusimt.cuda.selp, types.Boolean, types.Float, types.Float)
...
# Register warpsize and laneid for the cusimt.cuda module.
...
@lower(cusimt.cuda.clz, types.Integer)
...
@lower(cusimt.cuda.popc, types.Integer)
...
@lower(cusimt.cuda.ffs, types.Integer)
...
def lower_grid_group_sync_getattr(context, lower, target, grid_group_var):
    from cusimt.lowering_utilities import DeferredMethodCall
...
def _extract_shape_and_dtype(shape, dtype, alignment=None, alignas=None, **kwargs):
    ...
    # Support both 'alignment' (numba-cuda) and 'alignas' (cusimt) keywords
...
def lower_strides(
    ...
    from cusimt.lowering_utilities import int_of, index_of
...
@lower(cusimt.cuda.syncwarp)
@lower(cusimt.cuda.syncwarp, types.Number)
...
def register_vote_sync_lowerings():
    from cusimt import cuda
    from cusimt._mlir.dialects import nvvm, arith
    from cusimt._mlir.ir import IntegerType
...
def register_match_sync_lowerings():
    ...
    def match_all_sync_lowering(builder: MLIRLower, target, args, kwargs):
        ...
        # Return as Python tuple of MLIR values (cusimt stores tuples this way)
...
def register_activemask_lowering():
    from cusimt import cuda
    from cusimt._mlir.dialects import llvm as llvm_dialect
    from cusimt._mlir.ir import IntegerType
...
def cbrt(builder, target, args, kwargs):
    from cusimt._mlir.dialects import math
...
def ffs(builder: MLIRLower, target, args, kwargs):
    from cusimt.runtime import libdevice
...
def _lower_fp16_intrinsics():
    from cusimt._mlir.dialects import math, arith
