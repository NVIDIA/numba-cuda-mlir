...
import functools
import operator
from cusimt import lowering_utilities
...
from cusimt.mlir.dialect_exts import memref, scf, arith, tensor
...
from cusimt.lowering_utilities.linalg_lowering import (
    lower_np_binop,
    lower_matmul,
    lower_linalg_dot,
    lower_transpose,
...
from cusimt.numba_cuda.np.unsafe.ndarray import to_fixed_tuple
...
def np_any_cg(builder, target, args, kwargs):
    ...
    """
    assert len(args) == 1 and len(kwargs) == 0, "np.sum takes exactly one argument"
    from cusimt.lowering_utilities import constant, false
    ...
    with ir.InsertionPoint(block):
        from cusimt.lowering_utilities import not_equal, or_, and_, constant
...
def np_all_cg(builder, target, args, kwargs):
    ...
    from cusimt.lowering_utilities import constant, true
...
def lower_array_getitem(builder, target, args, kwargs):
    ...
    from cusimt.types import NestedArray

    if isinstance(array_numba_type, NestedArray):
        from cusimt.lowering.record import lower_nested_array_getitem_int
...
class MemRefSlice:
    ...
    def get(array: ir.Value, slice: Slice):
        from cusimt.lowering_utilities import try_extract_constant as extract
...
def lower_charseq_array_setitem_string(builder: MLIRLower, target, args, kwargs):
    ...
    from cusimt.lowering_utilities import GEP_DYNAMIC_INDEX, false as false_
...
def lower_array_setitem(builder: MLIRLower, target, args, kwargs):
    ...
    if isinstance(array_numba_type, NestedArray):
        from cusimt.lowering.record import lower_nested_array_setitem_int
...
def lower_array_setitem_tuple(builder, target, args, kwargs):
    ...
    array_numba_type = builder.get_numba_type(args[0].name)
    if isinstance(array_numba_type, NestedArray):
        from cusimt.lowering.record import lower_nested_array_setitem_tuple
...
def lower_array_tuple_getitem(builder: MLIRLower, target, args, kwargs):
    ...
    array_numba_type = builder.get_numba_type(args[0].name)
    if isinstance(array_numba_type, NestedArray):
        from cusimt.lowering.record import lower_nested_array_getitem_tuple
...
def np_nansum_cg(builder, target, args, kwargs):
    ...
    """
    from cusimt.lowering_utilities import add
...
def np_nanprod_cg(builder, target, args, kwargs):
    ...
    """
    from cusimt.lowering_utilities import mul
...
# which looks them up in cusimt.ufunc_db and delegates to these lowering functions.
...
def operator_neg_array_lower(builder, target, args, kwargs):
    ...
    from cusimt.mlir.dialect_exts import arith
