...
from cusimt._mlir.dialects import arith, scf
from cusimt._mlir import ir
from cusimt.mlir.dialect_exts import llvm

from cusimt.extending import overload_method, register_jitable
from cusimt.mlir_lowering_registry import MLIRLoweringRegistry
from cusimt.lowering_utilities import GEP_DYNAMIC_INDEX, true, false

from cusimt.numba_cuda.cpython.unicode import (
    _malloc_string,
    _set_code_point,
    _get_code_point,
    _kind_to_byte_width,
...
def _unicode_eq_lower(builder, target, args, kwargs):
    ...
    """
    from cusimt.lowering_utilities.string import materialize_string_constant_if_needed
...
@register_jitable
def _empty_string_cusimt(kind, length, is_ascii=0):
...
@register_jitable
def _ascii_upper_cusimt(data, res):
...
@register_jitable
def _ascii_lower_cusimt(data, res):
    for idx in range(len(data)):
        ch = _get_code_point(data, idx)
...
def unicode_upper(data):
    def impl(data):
        length = len(data)
        if length == 0:
            return _empty_string_cusimt(data._kind, length, data._is_ascii)
        res = _empty_string_cusimt(data._kind, length, 1)
        _ascii_upper_cusimt(data, res)
        return res
