...
from cusimt.errors import ensure_verifies
...
from cusimt.type_defs.float_types import BFloat16Type
...
from cusimt.mlir.dialect_exts import scf
from cusimt._mlir.extras.meta import region_op
from cusimt.logging import trace
from cusimt._mlir.dialects import bufferization
...
def _(a: types.Type, b: types.Type) -> types.Type:
    ...
    from cusimt.type_defs.float_types import SpecialFloatType
...
class UniTupleIterObject:
    ...
    def __init__(self, lower, tuple_memref: ir.Value, count: int, element_type: ir.Type):
        self._tuple_memref = tuple_memref
        self._count = count
    ...
    def next(self) -> IterResult:
        ...
        current_value = memref.load(self._tuple_memref, [index_as_index])
