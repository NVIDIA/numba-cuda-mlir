...
from cusimt.lowering_utilities import constant
...
def materialize_string_constant(gpu_module, pystr: str) -> ir.Value:
    ...
    """
    from cusimt.numba_cuda.cpython.unicode import compile_time_get_string_data

    databytes, length, kind, is_ascii, _ = compile_time_get_string_data(pystr)

    global_name = f"__cusimt_str_{next(_counter)}"
