...
def numba_sig_from_pyfunc(pysig: inspect.Signature) -> typing.Signature:
    ...
    if not all(isinstance(argtype, types.Type) for argtype in argtypes):
        raise TypeError("All arguments must have type annotations of type cusimt.types.Type")
    ...
    if retty and not isinstance(retty, types.NoneType) and retty is not None:
        raise TypeError("Return type must be None or cusimt.types.none")
...
def _(ty: ir.Type | ir.FunctionType) -> types.Type | typing.Signature:
    match ty:
        ...
        case ir.VectorType():
            from cusimt.type_defs.vector_types import VectorType
...
def _(ty: types.Type) -> ir.Type:
    ...
    from cusimt.numba_cuda.types.ext_types import Bfloat16, GridGroup as GridGroupClass
