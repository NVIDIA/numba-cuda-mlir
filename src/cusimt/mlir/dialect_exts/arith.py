...
from cusimt._mlir._mlir_libs._mlir import register_value_caster
from cusimt._mlir.dialects import complex as complex_dialect
from cusimt._mlir.dialects import math as math_dialect
from cusimt._mlir.dialects._arith_enum_gen import (
    _arith_cmpfpredicateattr,
    _arith_cmpipredicateattr,
)
from cusimt._mlir.dialects._ods_common import (
    get_op_result_or_op_results,
    get_op_result_or_value,
)
from cusimt._mlir.dialects.arith import *
from cusimt._mlir.ir import (
    Attribute,
    BF16Type,
    ComplexType,
    Context,
...
def _binary_op(
    ...
    *,
    ...
    if isinstance(lhs.dtype, FloatType):
        if op == "Xor":
            raise TypeError(
                f"unsupported operand type(s) for ^: '{type(lhs).__name__}' and "
                f"'{type(rhs).__name__}'"
            )
        if op == "Floordiv":
            # Python float // float is floor(a / b); arith has no floordivf — match
            # cusimt/lowering/math.py floordiv_cg: divf then math.floor.
            div_op = globals()["DivFOp"]
            div_val = lhs.__class__(div_op(lhs, rhs, loc=loc), dtype=lhs.dtype)
            return lhs.__class__(math_dialect.floor(div_val, loc=loc), dtype=lhs.dtype)
        op += "F"
    ...
    # MLIR ODS name is XOrIOp (capital O), not XorIOp, after "xor" -> capitalize -> "Xor" + "I".
    op_cls = f"{op}Op"
    if op_cls == "XorIOp":
        op_cls = "XOrIOp"
    op = globals()[op_cls]
...
def _pow_op(
    ...
    *,
    ...
    if isinstance(rhs, (float, int, bool)):
        rhs = lhs.coerce(rhs)
    elif isinstance(lhs, (float, int, bool)):
        lhs = rhs.coerce(lhs)

    if lhs.fold() and rhs.fold():
        klass = lhs.__class__
        return klass(operator.pow(lhs.literal_value, rhs.literal_value), fold=True)

    if isinstance(lhs.dtype, FloatType) and isinstance(rhs.dtype, FloatType):
        return lhs.__class__(math_dialect.powf(lhs, rhs, loc=loc), dtype=lhs.dtype)
    elif isinstance(lhs.dtype, FloatType) and isinstance(rhs.dtype, IntegerType):
        return lhs.__class__(math_dialect.fpowi(lhs, rhs, loc=loc), dtype=lhs.dtype)
    elif isinstance(lhs.dtype, IntegerType) and isinstance(rhs.dtype, IntegerType):
        assert lhs.type == rhs.type, f"{lhs=} {rhs=} must have the same type."
        from cusimt.mlir.dialect_exts.math import ipowi

        return lhs.__class__(ipowi(lhs, rhs, loc=loc), dtype=lhs.dtype)
    else:
        raise NotImplementedError(f"Unsupported pow operands: {lhs.type}, {rhs.type}")
...
def _rpow_op(
    ...
    *,
    ...
    return _pow_op(lhs, rhs, loc=loc)
...
class ArithValue(Value, metaclass=ArithValueMeta):
    ...
    __rxor__ = partialmethod(_rbinary_op, op="xor")
    __rpow__ = partialmethod(_rpow_op)
