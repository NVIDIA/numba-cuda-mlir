...
from cusimt._mlir.dialects import memref, arith, vector, builtin
from cusimt._mlir.dialects._ods_common import (
    get_op_result_or_op_results,
    MixedValues,
    _dispatch_mixed_values,
)
from cusimt._mlir.dialects.memref import (
    _is_static_int_like,
    _generated_subview,
...
class MemRefValue(Value):
    ...
    def __setitem__(self, idx, val):
        ...
        if all(isinstance(d, ScalarValue) for d in idx) and len(idx) == len(self.shape):
            ...
            if isinstance(val, ScalarValue):
            ...
            elif isinstance(val.type, VectorType):
                ...
                raise ValueError(f"unsupported value type for memref store: {val=}")
        else:
