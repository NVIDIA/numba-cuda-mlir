...
from cusimt._mlir.extras.meta import op_region_builder
...
try:
    from cusimt._mlir.ir import TypeID
except ImportError:
...
def get_user_code_loc(user_base: Optional[Path] = None):
    ...
    cusimt_mlir_root_path = Path(__file__).parent
    ...
    while prev_frame.f_back and (
        is_relative_to(Path(prev_frame.f_code.co_filename), cusimt_mlir_root_path)
...
def _update_caller_vars(previous_frame, args: Sequence, replacements: Sequence):
    ...
    arg_var_names = [_get_previous_frame_idents(arg, previous_frame) for arg in args]
    for i, names in enumerate(arg_var_names):
