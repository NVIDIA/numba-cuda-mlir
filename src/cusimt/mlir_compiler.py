...
import cusimt.mlir_lowering as lowering
import cusimt.mlir_optimization as opt
from cusimt.decorators import mlir_jit
from cusimt.ast_transforms import apply_ast_transforms
...
def _get_compiler_class(targetoptions: Dict[str, Any]):
    class MLIRCompiler(CompilerBase):
        def define_pipelines(self):
            ...
            # Replace passes with cusimt-specific versions:
            # - LiteralUnroll -> CusimtLiteralUnroll (supports both literal_unroll versions)
            # - InlineInlinables -> CusimtInlineInlinables (skips self-recursive functions)
            modified_passes = []
            for impl, desc in untyped_passes.passes:
                if impl is LiteralUnroll:
                    modified_passes.append((CusimtLiteralUnroll, desc))
                elif impl is InlineInlinables:
                    modified_passes.append((CusimtInlineInlinables, desc))
                else:
...
def compile_mlir(pyfunc, return_type, args, targetoptions: Dict[str, Any]):
    from cusimt.install_registry import register_lowering
...
class numba_mlir:
    """
    NUMBA MLIR Namespace
    """

    _description_ = "<numba-mlir>"
    ...
    @staticmethod
    def jit(
        signature_or_function: str | AnyCallable[PS] | None = None, **kws
    ) -> Callable[[AnyCallable[PS]], MLIRDispatcher] | MLIRDispatcher:
        """JIT decorator"""
        # Emit warnings for incompatible options early (before returning decorator)
        # This is needed so the warning is emitted when cuda.jit(debug=True, opt=True)
        # is called, even if no function is provided yet.
        debug = kws.get("debug")
        if debug is None:
        ...
        if debug and lineinfo:
            ...
            warnings.warn(NumbaInvalidConfigWarning(msg))
        if debug and opt:
            msg = (
        ...
        if callable(signature_or_function):
            return mlir_jit(signature_or_function, **kws)
        else:
            # Pass signature to mlir_jit when decorating
            return lambda func: mlir_jit(signature_or_function, **kws)(func)
