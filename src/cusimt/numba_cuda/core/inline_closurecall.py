...
from cusimt.numba_cuda.core.imputils import impl_ret_untracked
from cusimt.numba_cuda.extending import intrinsic
from cusimt.numba_cuda.typing import signature

from cusimt.numba_cuda.core import postproc, rewrites
from cusimt.numba_cuda.np.unsafe.ndarray import empty_inferred as unsafe_empty_inferred
import numpy as np
import operator
...
class InlineClosureCallPass:
    ...
    def run(self):
        ...
        if enable_inline_arraycall:
            ...
            for k, _ in sorted(sized_loops, key=lambda tup: tup[1], reverse=True):
                visited.append(k)
                if guard(
                    _inline_arraycall,
                    self.func_ir,
    ...
    def _inline_closure(self, work_list, block, i, func_def):
        require(isinstance(func_def, ir.expr_types) and func_def.op == "make_function")
        inline_closure_call(
            self.func_ir,
            self.func_ir.func_id.func.__globals__,
            block,
...
def check_reduce_func(func_ir, func_var):
    ...
    if isinstance(reduce_func, ir.freevar_types) or isinstance(reduce_func, ir.global_types):
        if HAS_NUMBA:
...
class InlineWorker:
    ...
    def __init__(
        ...
        from cusimt.numba_cuda.compiler import DefaultPassBuilder
    ...
    def run_untyped_passes(self, func, enable_ssa=False):
        ...
        """
        from cusimt.numba_cuda.core.compiler import StateDict, _CompileStatus
        from cusimt.numba_cuda.core.untyped_passes import ExtractByteCode
        from cusimt.numba_cuda.core import bytecode
    ...
    def update_type_and_call_maps(self, callee_ir, arg_typs):
        ...
        from cusimt.numba_cuda.core.ssa import reconstruct_ssa
        from cusimt.numba_cuda.core.typed_passes import PreLowerStripPhis
        ...
        from cusimt.numba_cuda.core import typed_passes, ir_utils
...
def inline_closure_call(
    ...
    if typingctx:
        from cusimt.numba_cuda.core import typed_passes
...
def _get_callee_args(call_expr, callee, loc, func_ir):
    ...
    if isinstance(callee, pytypes.FunctionType):
        ...
        if callee_defaults:
            ...
            elif isinstance(callee_defaults, ir.var_types) or isinstance(callee_defaults, str):
...
def _find_iter_range(func_ir, range_iter_var, swapped):
    ...
    require(isinstance(range_iter_def, ir.expr_types) and range_iter_def.op == "getiter")
...
def length_of_iterator(typingctx, val):
    ...
    if isinstance(val, types.RangeIteratorType):
        ...
        def codegen(context, builder, sig, args):
            (value,) = args
            from cusimt.numba_cuda.cpython.rangeobj import range_impl_map
        ...
        def codegen(context, builder, sig, args):
            ...
            from cusimt.numba_cuda.cpython.listobj import ListIterInstance
        ...
        def codegen(context, builder, sig, args):
            ...
            from cusimt.numba_cuda.np.arrayobj import make_array
...
def _inline_arraycall(func_ir, cfg, visited, loop, swapped, enable_prange=False, typed=False):
    ...
    require(isinstance(list_var_def, ir.expr_types) and list_var_def.op == "build_list")
    ...
    stmts.extend(_new_definition(func_ir, var, array_var, loc) for var in removed)
...
def _fix_nested_array(func_ir):
    ...
    def fix_array_assign(stmt):
        ...
        require(isinstance(size_tuple_def, ir.expr_types) and size_tuple_def.op == "build_tuple")
            ...
            if removed_var in state.list_items:
                ...
                if state.list_vars == []:
                    ...
                    for inst in state.stmts:
                        ...
                        ) or (isinstance(inst, ir.del_types) and inst.value in state.dead_vars):
                            continue
                        body.append(inst)
                    state.stmts = body
                    state.dead_vars = []
