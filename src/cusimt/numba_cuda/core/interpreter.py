...
from cusimt.numba_cuda.core import config, ir
from cusimt.numba_cuda.errors import UnsupportedBytecodeError
from cusimt.numba_cuda.core.errors import (
    NotDefinedError,
    error_extras,
)
from cusimt.numba_cuda.core import ir_utils
from cusimt.numba_cuda.utils import (
    PYVERSION,
    BINOPS_TO_OPERATORS,
    INPLACE_BINOPS_TO_OPERATORS,
)
from cusimt.numba_cuda.utils import _lazy_pformat
from cusimt.numba_cuda.core.byteflow import Flow, AdaptDFA, AdaptCFA, BlockKind
from cusimt.numba_cuda.core.unsafe import eh
from cusimt.numba_cuda.cpython.unsafe.tuple import unpack_single_tuple


if PYVERSION in ((3, 12), (3, 13), (3, 14)):
    # Operands for CALL_INTRINSIC_1
    from cusimt.numba_cuda.core.byteflow import CALL_INTRINSIC_1_Operand as ci1op
...
def _call_function_ex_replace_kws_large(
    ...
    while search_start <= search_end:
        ...
        if not (
            isinstance(const_stmt, ir.assign_types) and isinstance(const_stmt.value, ir.const_types)
        ):
...
def _call_function_ex_replace_args_large(
    ...
    search_start = 0
    total_args = []
    if isinstance(vararg_stmt, ir.assign_types) and isinstance(vararg_stmt.value, ir.var_types):
            ...
            def find_postive_region(blk):
                ...
                for idx in reversed(range(len(blk.body))):
                    ...
                    if isinstance(stmt, ir.assign_types):
                        value = stmt.value
                        if isinstance(value, ir.expr_types) and value.op == "list_to_tuple":
    ...
    def _remove_unused_temporaries(self):
        ...
        for inst in self.current_block.body:
            ...
            if (
                ...
                # later assignments
                if isinstance(inst.value, ir.var_types) and inst.value.name in replaced_var:
                    inst.value = replaced_var[inst.value.name]
