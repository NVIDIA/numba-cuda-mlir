...
from cusimt.numba_cuda.utils import UNARY_BUILTINS_TO_OPERATORS, OPERATORS_TO_BUILTINS
...
from cusimt.numba_cuda.core import consts
    ...
    def strformat(self, nlines_up=2):
        ...
        if lines and use_line > 0:
            ...
            def count_spaces(string):
                return sum(1 for _ in itertools.takewhile(str.isspace, str(string)))
        ...
        for label, block in self.blocks.items():
            ...
            if other_blk is not None:
                if block != other_blk:
                    ...
                    block_del = [x for x in block.body if isinstance(x, del_types)]
                    oth_del = [x for x in other_blk.body if isinstance(x, del_types)]
        ...
        if nofile:
            ...
            if config.HIGHLIGHT_DUMPS:
                try:
                    ...
                    from cusimt.numba_cuda.misc.dump_style import NumbaIRLexer as lexer
                    from cusimt.numba_cuda.misc.dump_style import by_colorscheme
