...
from cusimt.numba_cuda.core.analysis import compute_use_defs
...
def _loop_lift_modify_blocks(
    ...
    """
    from cusimt.numba_cuda.dispatcher import LiftedLoop
...
def with_lifting(func_ir, typingctx, targetctx, flags, locals):
    ...
    """
    from cusimt.numba_cuda.core import postproc

    def dispatcher_factory(func_ir, objectmode=False, **kwargs):
        from cusimt.numba_cuda.dispatcher import LiftedWith, ObjModeLiftedWith
            ...
            while to_visit:
                ...
                for stmt in blocks[block].body:
                    ...
                    if ir_utils.is_terminator(stmt):
                        to_visit.extend(t for t in stmt.get_targets() if t not in seen)
