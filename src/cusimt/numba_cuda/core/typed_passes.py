...
from cusimt.numba_cuda.core import typeinfer
from cusimt.numba_cuda.core import (
    errors,
)
from cusimt.numba_cuda.core import ir
from cusimt.numba_cuda import typing, types, lowering
...
from cusimt.numba_cuda.core.annotations import type_annotations
...
from cusimt.numba_cuda.core import postproc, rewrites, funcdesc, config
...
class BaseTypeInference(FunctionPass):
    ...
    def run_pass(self, state):
                ...
                for blk in interp.blocks.values():
                    for inst in blk.body:
                        if isinstance(inst, ir.return_types):
                            ...
                            if isinstance(inst.value, ir.expr_types) and inst.value.op == "cast":
        ...
        with fallback_context(
            state,
            'Function "%s" has invalid return type' % (state.func_id.func_name,),
        ):
...
class BaseNativeLowering(abc.ABC, LoweringPass):
    ...
    def run_pass(self, state):
        ...
        with fallback_context(state, msg):
            ...
            from cusimt.numba_cuda.compiler import _LowerResult  # TODO: move this
...
class NoPythonBackend(LoweringPass):
    ...
    def run_pass(self, state):
        ...
        from cusimt.numba_cuda.compiler import cuda_compile_result
...
class InlineOverloads(FunctionPass):
    ...
    def run_pass(self, state):
        ...
        if self._DEBUG:
            ...
            print("".center(80, "-"))
        from cusimt.numba_cuda.core.inline_closurecall import (
            InlineWorker,
            callee_ir_validator,
        )
    ...
    def _run_inliner(
        ...
        if not inline_type.is_always_inline:
            from cusimt.numba_cuda.typing.templates import _inline_info
...
class PreLowerStripPhis(FunctionPass):
    ...
    def _strip_phi_nodes(self, func_ir):
        ...
        for label, block in func_ir.blocks.items():
            ...
            term_loc = block.terminator.loc if block.is_terminated else block.loc
            for target, rhs in exporters[label]:
                ...
                assignments = [
                    stmt for stmt in newblk.find_insts(ir.assign_types) if stmt.target == rhs
                ]
                if assignments:
