...
from cusimt.numba_cuda.core import postproc, bytecode, transforms, inline_closurecall
from cusimt.numba_cuda.core import (
    errors,
)
from cusimt.numba_cuda.core import ir
from cusimt.numba_cuda import types
from cusimt.numba_cuda.core import consts, rewrites, config
from cusimt.numba_cuda.core.interpreter import Interpreter


from cusimt.numba_cuda.misc.special import literal_unroll
from cusimt.numba_cuda.core.analysis import dead_branch_prune
...
class WithLifting(FunctionPass):
    ...
    def run_pass(self, state):
        ...
        if withs:
            from cusimt.numba_cuda.compiler import compile_ir
            from cusimt.numba_cuda.core.compiler import _EarlyPipelineCompletion
...
class InlineInlinables(FunctionPass):
    ...
    def _do_work(self, state, work_list, block, i, expr, inline_worker):
        ...
        from cusimt.numba_cuda.core.options import InlineOptions
                    ...
                    if expr.op == "getiter":
                        ...
                        if isinstance(defn, ir.global_types):
                            if expr.func.is_temp:
                                deps.add(expr.func)
                elif isinstance(rhs, ir.global_types) and rhs.value in self._supported_globals:
                    startpt = assign
                ...
                if isinstance(stmt, ir.assign_types):
                    if isinstance(stmt.value, ir.expr_types):
                        if stmt.value.op == "make_function":
                            ...
                            ok = False
                            if kw_default is None or isinstance(kw_default, ir.const_types):
                                ok = True
                            elif isinstance(kw_default, tuple):
                                ok = all(
                                    [isinstance(getdef(x), ir.const_types) for x in kw_default]
                                )
    ...
    def run_pass(self, state):
        ...
        for blk in func_ir.blocks.values():
            ...
            for call in calls:
                ...
                if glbl and (
                    isinstance(glbl, ir.global_types) or isinstance(glbl, ir.freevar_types)
                ):
                    ...
                    if glbl.value is literal_unroll:
                        ...
                        if isinstance(to_unroll, ir.expr_types) and to_unroll.op == "build_list":
                            ...
                            asgn.value = tup
                            mutated = True
                        elif isinstance(to_unroll, ir.expr_types) and to_unroll.op == "build_tuple":
        ...
        def collect_literal_unroll_info(literal_unroll_loops):
            ...
            for loop, literal_unroll_call in literal_unroll_loops.items():
                ...
                for lbli in loop.body:
                    ...
                    for stmt in blk.body:
                        if isinstance(stmt, ir.assign_types):
                            if isinstance(stmt.value, ir.expr_types) and stmt.value.op == "getitem":
...
class IterLoopCanonicalization(FunctionPass):
        ...
        for iternext in iternexts:
            ...
            if getattr(phi, "op", False) == "getiter":
                if partial_typemap:
                    ...
                    if func is None or not isinstance(func, ir.global_types + ir.freevar_types):
                        return False
    ...
    def transform(self, loop, func_ir, cfg):
        ...
        for stmt in entry_block.body:
            if isinstance(stmt, ir.assign_types):
                if isinstance(stmt.value, ir.expr_types) and stmt.value.op == "getiter":
                    break
            idx += 1
        else:
        ...
        for block in func_ir.blocks.values():
            for assign in block.find_insts(ir.assign_types):
                ...
                if isinstance(value, ir.expr_types) and value.op == "call":
                    ...
                    if not (isinstance(fn, ir.global_types) and fn.name in accepted_functions):
                        continue
                    ...
                    for arg in value.args:
                        ...
                        assert len(iv) == 1  # SSA!
                        if isinstance(iv[0], ir.expr_types) and iv[0].op == "phi":
                            msg = (
