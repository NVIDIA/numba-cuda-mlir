...
from cusimt.numba_cuda.extending import overload_method, overload
...
def getitem_typed(context, builder, sig, args):
    ...
    if len(tupty) == 0:
        ...
        with builder.if_then(cgutils.true_bit):
            context.fndesc.call_conv.return_user_exc(builder, IndexError, errmsg_oob)
        ...
        for i in range(tupty.count):
            ...
            with builder.goto_block(bbi):
                ...
                # End Dragon warning...
                DOCAST = (
                    context.typing_context.unify_types(sig.args[0][i], sig.return_type)
                    == sig.return_type
                )
                if DOCAST:
