...
from cusimt import _mviewbuf as mviewbuf
from cusimt.numba_cuda.core import config
from cusimt.numba_cuda import utils, serialize
from .error import CudaSupportError, CudaDriverError
from .mappings import FILE_EXTENSION_MAP
from .linkable_code import LinkableCode, LTOIR, Fatbin, Object
from cusimt.numba_cuda.utils import cached_file_read
...
from cusimt.numba_cuda._compat import CUDA_CORE_GT_0_6
...
def _have_nvjitlink():
    ...
    try:
        if nvjitlink_internal._inspect_function_pointer("__nvJitLinkVersion") == 0:
            return False
        try:
...
class Driver:
    ...
    def _find_api(self, fname):
        ...
        for variant in variants:
            if (value := getattr(self.lib, f"{fname}{variant}", None)) is not None:
                return value
...
class Device:
    ...
    def get_primary_context(self):
        ...
        if self.compute_capability < MIN_REQUIRED_CC:
            raise CudaSupportError(f"{self} has compute capability < {MIN_REQUIRED_CC}")
...
class Context:
    ...
    def reset(self):
        ...
        from cusimt.numba_cuda.memory_management.nrt import rtsys
    ...
    def create_module_image(self, image, setup_callbacks=None, teardown_callbacks=None):
        module = load_module_image(self, image, setup_callbacks, teardown_callbacks)
        key = module.handle
    ...
    def create_stream(self):
        ...
        return Stream(handle, finalizer=_stream_finalizer(self.deallocations, handle))
    ...
    def create_event(self, timing=True):
        ...
        return Event(handle, finalizer=_event_finalizer(self.deallocations, handle))
...
FuncAttr = namedtuple("FuncAttr", ["regs", "shared", "local", "const", "maxthreads"])
...
def inspect_obj_content(objpath: str):
    ...
    try:
        out = subprocess.run(["cuobjdump", objpath], check=True, capture_output=True)
...
def _stream_handle(stream):
    ...
    if not isinstance(stream, allowed):
        raise TypeError("Expected a Stream object or 0, got %s" % type(stream).__name__)
