...
from cusimt.numba_cuda import serialize, utils
...
from cusimt.numba_cuda.typeconv.rules import default_type_manager
from cusimt.numba_cuda.typing.templates import fold_arguments
from cusimt.numba_cuda.typing.typeof import Purpose, typeof

from cusimt.numba_cuda import typing, types
from cusimt.numba_cuda.types import ext_types
from cusimt.numba_cuda.api import get_current_device
from cusimt.numba_cuda.args import wrap_arg
from cusimt.numba_cuda.core.bytecode import get_code_object
...
from cusimt.numba_cuda.locks import module_init_lock
from cusimt.numba_cuda.core.caching import Cache, CacheImpl, NullCache
from cusimt.numba_cuda.descriptor import cuda_target
from cusimt.numba_cuda.errors import (
    missing_launch_config_msg,
    normalize_kernel_dimensions,
)
from cusimt.numba_cuda.cudadrv.linkable_code import LinkableCode
from cusimt.numba_cuda.cudadrv.devices import get_context
from cusimt.numba_cuda.memory_management.nrt import rtsys, NRT_LIBRARY
import cusimt.numba_cuda.core.event as ev

# from cusimt.numba_cuda.cext import _dispatcher
...
class _Kernel(serialize.ReduceMixin):
    ...
    def __init__(
        ...
        if shared_memory_carveout is not None:
            self.shared_memory_carveout = self._parse_carveout(shared_memory_carveout)
        else:
        ...
        def link_to_library_functions(library_functions, library_path, prefix=None):
            """
            ...
            if prefix is not None:
                library_functions = [f"{prefix}{fn}" for fn in library_functions]
...
class _DispatcherBase:  # (_dispatcher.Dispatcher):
    ...
    def inspect_types(
        ...
        if not pretty:
            ...
            from cusimt.numba_cuda.core.annotations.pretty_annotate import Annotate
...
class CUDADispatcher(serialize.ReduceMixin, _MemoMixin, _DispatcherBase):
    ...
    def call(self, args, griddim, blockdim, stream, sharedmem):
        ...
        """
        raise RuntimeError("cuSIMT should not launch Numba-CUDA kernels")
    ...
    def add_overload(self, kernel, argtypes):
        ...
        # We don't insert the overload into _dispatcher because it doesn't
        # exist, but we keep the overloads dict for resolution of functions
        # during compilation.
