...
from cusimt.numba_cuda.core.imputils import impl_ret_borrowed, impl_ret_new_ref
from cusimt.numba_cuda.typing import signature
from cusimt.numba_cuda.extending import intrinsic, overload, register_jitable
...
from cusimt.numba_cuda.np import numpy_support as np_support
    ...
    if isinstance(left, types.Array) and isinstance(right, types.Array):
        ...
        def _impl(typingcontext, left, right):
            ...
            if ndims == (2, 2):
                ...
                raise TypingError(("%s: inputs must have compatible dimensions") % name)
...
def vdot(a, b):
    ...
    if isinstance(a, types.Array) and isinstance(b, types.Array):
        ...
        if a.layout not in "CF" or b.layout not in "CF":
            warnings.warn(
                "np.vdot() is faster on contiguous arrays, called on %s" % ((a, b),),
                NumbaPerformanceWarning,
            )
...
def dot_3(a, b, out):
    ...
    if (
        ...
        if a.layout not in "CF" or b.layout not in "CF" or out.layout not in "CF":
            warnings.warn(
                "np.vdot() is faster on contiguous arrays, called on %s" % ((a, b),),
                NumbaPerformanceWarning,
            )
