...
from cusimt.numba_cuda.np import arrayobj
from cusimt.numba_cuda.np import ufunc_db
from cusimt.numba_cuda.np.ufunc.sigparse import parse_signature
...
from cusimt.numba_cuda.np.arrayobj import _getitem_array_generic
from cusimt.numba_cuda.typing import npydecl
...
def register_ufuncs(ufuncs, lower):
    ...
    for _op_map in (npydecl.NumpyRulesInplaceArrayOperator._op_map,):
        for op, ufunc_name in _op_map.items():
            ...
            kernel = kernels[ufunc]
            if ufunc.nin == 1:
                register_unary_operator_kernel(op, ufunc, kernel, lower, inplace=True)
            elif ufunc.nin == 2:
