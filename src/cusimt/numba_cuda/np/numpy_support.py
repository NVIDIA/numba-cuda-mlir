...
# re-export
from cusimt.numba_cuda.cgutils import is_nonelike  # noqa: F401
...
def strides_from_shape(
    *,
    ...
    strides = tuple(
        map(
            itemsize.__mul__,
            itertools.accumulate(transform(shape[limits]), operator.mul, initial=1),
        )
    )
    if f_contiguous:
        return strides
...
def carray(ptr, shape, dtype=None):
    ...
    """
    from cusimt.numba_cuda.typing.ctypes_utils import from_ctypes
