...
class Record(Type):
    ...
    *aligned* is a boolean; whether the record is ABI aligned.
    ...
    def make_c_struct(cls, name_types):
        ...
        """
        from cusimt.numba_cuda.core.registry import cpu_target
    ...
    @property
    def dtype(self):
        from cusimt.numba_cuda.np.numpy_support import as_struct_dtype
    ...
    def can_convert_to(self, typingctx, other):
        ...
        """
        from cusimt.numba_cuda.core.errors import NumbaExperimentalFeatureWarning
...
class PolynomialType(Type):
    def __init__(self, coef, domain=None, window=None, n_args=1):
        super().__init__(name=f"PolynomialType({coef}, {domain}, {domain}, {n_args})")
        self.coef = coef
        self.domain = domain
        self.window = window
