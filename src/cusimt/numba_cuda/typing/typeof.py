...
def typeof_impl(val, c):
    ...
    from cusimt.numba_cuda.typing import cffi_utils
...
def _typeof_buffer(val, c):
    from cusimt.numba_cuda.typing import bufproto
...
@functools.lru_cache
def _typeof_cuda_array_interface_cached(*, dtype, shape, strides, readonly: bool):
    # Determine layout
    ndim = len(shape)
    if not ndim:
        layout = "C"
...
def typeof_strided_memory_view(val, c):
    ...
    if raw_layout.is_contiguous_c:
        ...
        raise ValueError("Unsupported StridedMemoryView layout; must be contiguous")
    return types.Array(
        dtype=numpy_support.from_dtype(val.dtype),
        ndim=len(val.shape),
        layout=layout,
