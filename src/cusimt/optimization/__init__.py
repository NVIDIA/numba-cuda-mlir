...
import functools
from cusimt._mlir.dialects import gpu, arith
...
def fixup_nvvm_arg_attrs(op: gpu.GPUFuncOp):
    if "cusimt.arg_attrs" in op.attributes:
        attrs = [[j for j in i] for i in op.attributes["cusimt.arg_attrs"]]
        if all(len(i) == 0 for i in attrs):
            return
        orig_arg_types = op.attributes["cusimt.orig_arg_types"]
        arg_attrs = op.attributes["cusimt.arg_attrs"]
        new_arg_attrs = []
        for i, arg_attr in enumerate(arg_attrs):
            new_arg_attr = {}
            for namedattr in arg_attr:
                if "cusimt.grid_constant" == namedattr.name:
        ...
        del op.attributes["cusimt.arg_attrs"]
        del op.attributes["cusimt.orig_arg_types"]
