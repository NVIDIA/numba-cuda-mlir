...
_registered_signatures = set()


def _signature_key(args):
    try:
        hash(args)
        return args
    except TypeError:
        return tuple(repr(arg) for arg in args)


def lower(*args):
    key = _signature_key(args)
    if key in _registered_signatures:
        return lambda fn: fn
    _registered_signatures.add(key)
    return _raw_lower(*args)
