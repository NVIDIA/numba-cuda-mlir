...
from numba_cuda_mlir.numba_cuda.misc.appdirs import AppDirs
...
class IndexDataCacheFile:
    ...
    def __init__(self, cache_path, filename_base, source_stamp):
        ...
        self._version = numba_cuda_mlir.__version__
...
class _UserWideCacheLocator(_SourceFileBackedLocatorMixin, _CacheLocator):
    ...
    def __init__(self, py_func, py_file):
        self._py_file = py_file
        self._lineno = py_func.__code__.co_firstlineno
        appdirs = AppDirs(appname="numba_cuda_mlir", appauthor=False)
...
class _IPythonCacheLocator(_CacheLocator):
    ...
    def get_cache_path(self):
        ...
        return os.path.join(get_ipython_cache_dir(), "numba_cuda_mlir_cache")
