...
from numba_cuda_mlir.numba_cuda.core import config, ir
from numba_cuda_mlir.numba_cuda.errors import UnsupportedBytecodeError
from numba_cuda_mlir.numba_cuda.core.errors import (
    NotDefinedError,
    error_extras,
)
from numba_cuda_mlir.numba_cuda.core import ir_utils
from numba_cuda_mlir.numba_cuda.utils import (
    PYVERSION,
    BINOPS_TO_OPERATORS,
    INPLACE_BINOPS_TO_OPERATORS,
    _lazy_pformat,
)
from numba_cuda_mlir.numba_cuda.core.byteflow import Flow, AdaptDFA, AdaptCFA, BlockKind
from numba_cuda_mlir.numba_cuda.core.unsafe import eh
from numba_cuda_mlir.numba_cuda.cpython.unsafe.tuple import unpack_single_tuple


if PYVERSION in ((3, 12), (3, 13), (3, 14), (3, 15)):
    # Operands for CALL_INTRINSIC_1
    from numba_cuda_mlir.numba_cuda.core.byteflow import (
        CALL_INTRINSIC_1_Operand as ci1op,
    )
...
class Interpreter:
    ...
    def interpret(self, bytecode):
        ...
        if PYVERSION in ((3, 9), (3, 10), (3, 11), (3, 12), (3, 13), (3, 14), (3, 15)):
    ...
    def op_BINARY_OP(self, inst, op, lhs, rhs, res):
        if op == "[]":
            # Python 3.14+ folds the BINARY_SUBSCR body into BINARY_OP.
            lhs = self.get(lhs)
            rhs = self.get(rhs)
    ...
    if PYVERSION in ((3, 14), (3, 15)):
        ...
        def op_LOAD_COMMON_CONSTANT(self, inst, res, idx):
            ...
            if const in (AssertionError, NotImplementedError, tuple, all, any, list, set):
