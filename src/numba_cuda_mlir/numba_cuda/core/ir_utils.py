...
import numba_cuda_mlir.numba_cuda
from numba_cuda_mlir.numba_cuda import types
from numba_cuda_mlir.numba_cuda.core import ir
from numba_cuda_mlir.numba_cuda import typing
from numba_cuda_mlir.numba_cuda.core import analysis, postproc, rewrites, config
...
def remove_dels(blocks):
    """remove ir.Del nodes"""
    for block in blocks.values():
        block.body = [stmt for stmt in block.body if not isinstance(stmt, ir.Del)]
    return
...
def get_ir_of_code(glbls, fcode):
    ...
    from numba_cuda_mlir.numba_cuda.core.inline_closurecall import InlineClosureCallPass

    inline_pass = InlineClosureCallPass(
        ir, numba_cuda_mlir.numba_cuda.core.options.ParallelOptions(False), swapped
    )
    inline_pass.run()
    ...
    from numba_cuda_mlir.numba_cuda.core.untyped_passes import ReconstructSSA
