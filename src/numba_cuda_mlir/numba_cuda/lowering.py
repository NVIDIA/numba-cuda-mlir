...
from numba_cuda_mlir.numba_cuda import debuginfo, cgutils, utils, typing, types
...
from numba_cuda_mlir.numba_cuda.core.funcdesc import default_mangler
from numba_cuda_mlir.numba_cuda.core.environment import Environment
from numba_cuda_mlir.numba_cuda.core.analysis import compute_use_defs, must_use_alloca
from numba_cuda_mlir.numba_cuda.misc.firstlinefinder import get_func_def_lineno
from numba_cuda_mlir.numba_cuda.misc.coverage_support import get_registered_loc_notify
...
class Lower(BaseLower):
    ...
    def pre_block(self, block):
        from numba_cuda_mlir.numba_cuda.core.unsafe import eh
    ...
    def _lower_call_ObjModeDispatcher(self, fnty, expr, signature):
        from numba_cuda_mlir.numba_cuda.core.pythonapi import ObjModeUtils
    ...
    def _alloca_var(self, name, fetype):
        ...
        if (name not in self._singly_assigned_vars) or self._disable_sroa_like_opt:
    ...
    def storevar(self, value, name, argidx=None):
        ...
        # Store variable
        if name in self._singly_assigned_vars and not self._disable_sroa_like_opt:
    ...
    def delvar(self, name):
        ...
        # Out-of-order
        if name not in self._blk_local_varmap and not self._disable_sroa_like_opt:
...
class CUDALower(Lower):
    ...
    def storevar(self, value, name, argidx=None):
        ...
        if config.CUDA_DEBUG_POLY:
            ...
            if src_name in self.poly_var_typ_map:
                ...
                discriminant_i8 = llvm_ir.Constant(llvm_ir.IntType(8), discriminant_val)
    ...
    def pre_lower(self):
        ...
        if self.context.enable_debuginfo and self._disable_sroa_like_opt:
            ...
            self.poly_var_typ_map = {k: v for k, v in poly_map.items() if len(v) > 1}
