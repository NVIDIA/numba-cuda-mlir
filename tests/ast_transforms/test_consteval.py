...
from numba_cuda_mlir.cuda import consteval, jit
...
def test_consteval_target_options_chip():
    ...
    @numba_cuda_mlir.jit(chip="sm_90", experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
...
def test_consteval_target_options_fast_math():
    ...
    @numba_cuda_mlir.jit(fastmath=True, experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
...
def test_consteval_block_with_target_options():
    ...
    @numba_cuda_mlir.jit(chip="sm_80", experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
        with consteval():
