...
import cusimt
from cusimt.cuda import consteval, jit
from cusimt.ast_transforms import ConstevalError
...
def test_consteval_freevars():
    ...
    @jit(experimental_ast_transforms=True)
    def k(x):
...
def test_consteval_target_options_chip():
    ...
    from cusimt.cuda import current_target_options

    @cusimt.jit(chip="sm_90", experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
...
def test_consteval_target_options_fast_math():
    ...
    @cusimt.jit(fastmath=True, experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
...
def test_consteval_simt_current_target_options():
    """Test using cuda.simt.current_target_options() syntax."""
    import cuda.simt as cs

    @cs.jit(chip="sm_80", experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
        chip = consteval(cs.current_target_options()["chip"])
...
def test_consteval_block_with_target_options():
    ...
    @cusimt.jit(chip="sm_80", experimental_ast_transforms=True)
    def kernel(arr):
        i = cuda.threadIdx.x
        with consteval():
...
def test_consteval_runtime_error_without_ast_transforms():
    ...
    from cusimt.cuda import consteval as ce
...
def test_consteval_block_unit_basic():
    ...
    import ast
    from cusimt.ast_transforms.consteval import transform_consteval
...
def test_consteval_block_unit_variable_tracking():
    ...
    import ast
    from cusimt.ast_transforms.consteval import ConstevalTransformer
    from cusimt.ast_transforms.common import get_function_ast
