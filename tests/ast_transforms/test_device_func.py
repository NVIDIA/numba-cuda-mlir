...
def kernel_factory(M):  # M is CT-constant
    ...
    def kernel(x):
        ...
        for i in cs.consteval(range(config["N"] + 1)):
            cs.consteval(print(f"COMPTIME: unrolled loop iteration {i}"))
            if cs.consteval(config["assign_one"]):
                print("RUNTIME: assigning ", M, " to index: ", i)
                x[i] = M
            else:
                x[i] = 2
        epi = cs.consteval(cs.jit(epilogue_for(config["arch"])))
        epi(x)
