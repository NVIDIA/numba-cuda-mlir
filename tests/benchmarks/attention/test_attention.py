...
@cusimt_cuda.jit
def cusimt_simple_attention(X, Y, S, D):
    i = cusimt_cuda.blockIdx.x
    scores = cusimt_cuda.shared.array(0, dtype=cusimt_cuda.float32)
    inv_sqrt_d = 0.015625
...
def run_cusimt_version(h_X):
    d_X = cusimt_cuda.to_device(h_X)
    d_Y = cusimt_cuda.device_array(S * D, dtype=np.float32)
    cusimt_simple_attention[S, 1, 0, S * 4](d_X, d_Y, S, D)
...
def test_attention():
    ...
    cusimt_output = run_cusimt_version(h_X)
    verify_against_reference(reference, cusimt_output, tolerance=2e-2, name="cuSIMT")
...
def run_benchmark_main():
    ...
    start = time.perf_counter()
    cusimt_simple_attention.compile(sig)
    ...
    d_X = cusimt_cuda.to_device(h_X)
    d_Y = cusimt_cuda.device_array(S * D, dtype=np.float32)
    cusimt_simple_attention[S, 1, 0, shared_mem_size](d_X, d_Y, S, D)
    cusimt_cuda.synchronize()
