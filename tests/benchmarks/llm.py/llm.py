...
cx_warp_sum = cudax.warp.sum(cs.float32)
...
cx_warp_max = cudax.warp.reduce(cs.float32, max_op)
max_storage_bytes = cx_warp_max.temp_storage_bytes
cx_warp_files += cx_warp_max.files


@cs.jit("float32(float32)", device=True, fastmath=True)
def warp_sum(val):
    temp_storage = cs.shared.array(shape=sum_storage_bytes, dtype=cs.uint8)
    warp_output = cx_warp_sum(temp_storage, val)
    return cs.shfl_sync(0xFFFFFFFF, warp_output, 0)


@cs.jit("float32(float32)", device=True, fastmath=True)
def warp_max(val):
    temp_storage = cs.shared.array(shape=max_storage_bytes, dtype=cs.uint8)
...
def layernorm_forward_kernel3(out, mean, rstd, inp, weight, bias, N, C):
    ...
    idx = cs.blockIdx.x * meta_group_size + meta_group_rank
    ...
    m = cs.libdevice.fast_fdividef(warp_sum(sum), fp32(C))
    if thread_rank == 0:
        mean[idx] = m
    ...
    s = warp_sum(sum)
    s = cs.libdevice.rsqrtf(cs.libdevice.fast_fdividef(s, fp32(C)) + fp32(1e-5))
...
def softmax_forward_kernel5(out, inv_temperature, inp, flt_max, N, T):
    ...
    for i in range(int32(thread_rank), int32(pos_by_4), int32(warp_size)):
        ...
        for k in range(int32(4)):
            maxval = cs.libdevice.fmaxf(maxval, v[k])
    ...
    if (4 * pos_by_4 + thread_rank) <= own_pos:
        old_maxval = maxval
        maxval = cs.libdevice.fmaxf(maxval, x[4 * pos_by_4 + thread_rank])
        sumval *= math.exp(inv_temperature * (old_maxval - maxval))
...
@cs.jit("UniTuple(int32, 4)(int32, int32, int32, int32)", device=True, fastmath=True)
...
@cs.jit("void(float32[:], float32[:], int32, int32, int32, int32)", fastmath=True)
...
# @cs.jit('UniTuple(int32, 3)(int32, int32, int32)', device=True, fastmath=True)
@cs.jit(device=True, fastmath=True)
def i2n_2(idx, E1, E2):
    bt = idx // E1
    b = bt // E2
    t = bt % E2
...
@cs.jit("void(float32[:], float32[:], float32[:])", fastmath=True)
...
def prepare_softmax_blockwide_nofloat4(idx, inp, V, P):
    ...
    for i in range(V + cs.threadIdx.x - cs.blockDim.x, -1, -cs.blockDim.x):
        v = x[i]
        old_maxval = thread_maxval
        thread_maxval = cs.libdevice.fmaxf(thread_maxval, v)
    ...
    shared_maxval = cs.shared.array(shape=32, dtype=cs.float32)
    shared_sumval = cs.shared.array(shape=32, dtype=cs.float32)
    num_warps = cs.blockDim.x // 32
    warp_id = cs.threadIdx.x // 32
    lane_id = cs.threadIdx.x % 32
...
def fused_classifier_kernel3(logits, losses, targets, B, T, V, P):
    ...
    for i in range(int32(cs.threadIdx.x), int32(V), int32(cs.blockDim.x)):
...
def softmax_forward_kernel7(out, inp, N, C):
    ...
    maxval = -math.inf
    for i in range(tid, C, cs.blockDim.x * UNROLL_FACTOR):
        for u in range(UNROLL_FACTOR):
            maxval = cs.libdevice.fmaxf(maxval, x[min(C - 1, i + u * cs.blockDim.x)])
    ...
    if tid == 0:
        ...
        for i in range(1, warps_per_block):
            val = cs.libdevice.fmaxf(val, maxvals[i])
    ...
    sumval = fp32(0.0)
    for i in range(tid, C, cs.blockDim.x * UNROLL_FACTOR):
        reg_array = cs.local.array(shape=UNROLL_FACTOR, dtype=cs.float32)
        for u in range(UNROLL_FACTOR):
            reg_array[u] = x[min(C - 1, i + u * cs.blockDim.x)]
        for u in range(UNROLL_FACTOR):
    ...
    for i in range(tid, C, cs.blockDim.x * UNROLL_FACTOR):
        reg_array = cs.local.array(shape=UNROLL_FACTOR, dtype=cs.float32)
        for u in range(UNROLL_FACTOR):
            reg_array[u] = y[min(C - 1, i + u * cs.blockDim.x)]
        for u in range(UNROLL_FACTOR):
            if (i + u * cs.blockDim.x) < C:
                y[i + u * cs.blockDim.x] = reg_array[u] / sum
...
def layernorm_backward_kernel(
    ...
    for i in range(int32(thread_rank), int32(C), int32(warp_size)):
        ...
        cs.atomic.add(dbias, i, dout_bt[i])
        # gradient contribution to weight
        cs.atomic.add(dweight, i, norm_bti * dout_bt[i])
...
@cs.jit("void(float32[:], float32[:], float32[:], int32)", fastmath=True)
def gelu_backward_kernel(dinp, inp, dout, N):
    ...
    if i < N:
        ...
        tanh_out = cs.libdevice.tanhf(tanh_arg)
        coshf_out = cs.libdevice.coshf(tanh_arg)
...
def softmax_autoregressive_backward_kernel(dpreatt, datt, att, B, T, C, scale):
    ...
    meta_group_rank = cs.threadIdx.x // warp_size
    thread_rank = cs.threadIdx.x % warp_size
    block_acc = cs.shared.array(shape=32, dtype=cs.float32)

    idx = cs.blockIdx.y
    # go through blocks in reverse order, so the slowest block starts first
    t0 = T - 1 - T_per_block * cs.blockIdx.x
    att = att[idx * T * T :]
    datt = datt[idx * T * T :]
    ...
    for to in range(int32(0), int32(T_per_block)):
        ...
        local_sum = fp32(0.0)
        for t2 in range(int32(cs.threadIdx.x), int32(t + 1), int32(BlockSize)):
            local_sum += att_bth[t2] * datt_bth[t2]
            # if cs.blockIdx.x == 0 and cs.blockIdx.y == 0 and cs.threadIdx.x == 0:
        ...
        for t3 in range(int32(cs.threadIdx.x), int32(t + 1), int32(BlockSize)):
...
def encoder_backward_kernel(dwte, dwpe, dout, inp, B, T, C):
    ...
    if idx < N:
        ...
        dwte_ix = dwte[ix * C + c :]
        dwpe_tc = dwpe[t * C + c :]
        cs.atomic.add(dwte_ix, 0, fp32(dout_btc[0]))
        cs.atomic.add(dwpe_tc, 0, fp32(dout_btc[0]))
...
@cs.jit("float32(float32, float32, float32)", device=True, fastmath=True)
def lerp_(start, end, weight):
    return cs.libdevice.fma(weight, end, cs.libdevice.fma(-weight, start, start))
