...
@cusimt_cuda.jit
def cusimt_vector_add(a, b, c, n):
    idx = cusimt_cuda.blockIdx.x * cusimt_cuda.blockDim.x + cusimt_cuda.threadIdx.x
    if idx < n:
        c[idx] = a[idx] + b[idx]
...
def run_cusimt_scalar(a, b):
    n = len(a)
    a_device = cusimt_cuda.to_device(a)
    b_device = cusimt_cuda.to_device(b)
    c_device = cusimt_cuda.device_array(n, dtype=np.float32)
    threads = 1024
    blocks = (n + threads - 1) // threads
    cusimt_vector_add[blocks, threads](a_device, b_device, c_device, n)
    cusimt_cuda.synchronize()
    return c_device.copy_to_host()


def run_cusimt_vectorized(a, b):
    n = len(a)
    a_device = cusimt_cuda.to_device(a)
    b_device = cusimt_cuda.to_device(b)
    c_device = cusimt_cuda.device_array(n, dtype=np.float32)
    ...
    cusimt_vector_add_vectorized[blocks, threads](a_device, b_device, c_device, n)
    cusimt_cuda.synchronize()
    return c_device.copy_to_host()
...
def test_vector_addition():
    ...
    expected = a + b
    cusimt_result = run_cusimt_scalar(a, b)
    assert np.allclose(cusimt_result, expected), "cuSIMT scalar verification failed"


def test_vector_addition_vectorized():
    a, b = get_input_data(TEST_N)
    expected = a + b
    cusimt_result = run_cusimt_vectorized(a, b)
    assert np.allclose(cusimt_result, expected), "cuSIMT vectorized verification failed"
...
def run_benchmark_vectorized():
    ...
    start = time.perf_counter()
    cusimt_vector_add_vectorized.compile(sig)
    cusimt_compile_time = (time.perf_counter() - start) * 1000
