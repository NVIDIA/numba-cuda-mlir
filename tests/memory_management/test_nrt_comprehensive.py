...
def reset_rtsys():
    ...
    from cusimt.memory_management import rtsys
...
class TestNrtLinking:
    ...
    def test_nrt_detect_linked_ptx_file(self):
        ...
        from cusimt.memory_management.nrt import get_include
        from cusimt.cuda.cudadrv.nvrtc import compile
        from cusimt.cuda.cudadrv.linkable_code import PTXSource
        from cusimt.cuda import get_current_device
...
class TestNrtStatistics:
    ...
    def setup_method(self):
        ...
        from cusimt.memory_management import rtsys
        from cusimt.cuda import default_stream

        self._stream = default_stream()
        # Store current stats state
        rtsys.ensure_initialized(self._stream)
        self._stats_state = rtsys.memsys_stats_enabled(self._stream)

    def teardown_method(self):
        """Restore stats state after each test."""
        from cusimt.memory_management import rtsys

        # Restore previous stats state
