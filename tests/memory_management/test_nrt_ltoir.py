...
from numba_cuda_mlir.memory_management import (
    compile_nrt_ltoir,
    get_include,
    needs_nrt_linking,
)
...
def test_nrt_disk_cache(tmp_path, monkeypatch):
    monkeypatch.setenv("NUMBA_CUDA_MLIR_CACHE_DIR", str(tmp_path))
...
def test_nrt_disk_cache_disabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUMBA_CUDA_MLIR_DISABLE_CACHE", "1")
    compile_nrt_ltoir.cache_clear()
