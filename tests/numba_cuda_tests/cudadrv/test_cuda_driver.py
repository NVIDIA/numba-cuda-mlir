...
from cusimt.numba_cuda.cudadrv.driver import host_to_device, device_to_host, driver
...
from cusimt.numba_cuda.cudadrv import devices, nvrtc
from cusimt.testing import NumbaCUDATestCase
from cusimt.numba_cuda.testing import cc_X_or_above, skip_on_cudasim
from cusimt.numba_cuda.tests.support import override_config
...
class TestCudaDriver(NumbaCUDATestCase):
    ...
    def test_cuda_driver_stream_operations(self):
        from cusimt.numba_cuda.cudadrv.driver import _to_core_stream
    ...
    @pytest.mark.xfail(
        reason=(
            "cuSIMT still routes native DeviceNDArray launch handling through "
            "__cuda_array_interface__, unlike numba.cuda"
        )
    )
        ...
        for value in carveout_values:
            with self.subTest(carveout=value):
                try:
                    ...
                    self.fail(f"set_shared_memory_carveout({value}) failed: {e}")
...
class TestDevice(NumbaCUDATestCase):
...
class TestAcceleratedArchitecture(NumbaCUDATestCase):
    @pytest.mark.skipif(
        ...
        reason="tensormap.replace requires sm_9x",
    )
