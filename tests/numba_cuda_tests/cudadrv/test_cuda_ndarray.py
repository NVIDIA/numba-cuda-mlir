...
from cusimt.numba_cuda.cudadrv import devicearray
import cuda.simt as cuda
from cusimt.testing import IS_NUMPY_2, NumbaCUDATestCase
from cusimt.numba_cuda.testing import skip_on_cudasim

import pytest


class TestCudaNDArray(NumbaCUDATestCase):
    ...
    def test_device_array_vectors(self):
        ...
        # ints still work
        dary = cuda.device_array(shape=np.array([10, 10]), dtype=np.bool_)
        self.assertEqual(dary.shape, (10, 10))
        dary = cuda.device_array(shape=[10, 10], dtype=np.bool_)
        self.assertEqual(dary.shape, (10, 10))
...
class TestArrayMethod(NumbaCUDATestCase):
...
class TestRecarray(NumbaCUDATestCase):
...
class TestCoreContiguous(NumbaCUDATestCase):
