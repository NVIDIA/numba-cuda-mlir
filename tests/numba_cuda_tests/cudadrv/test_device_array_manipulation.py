...
"""
DeviceArray behaviour and memory-management tests.

These tests validate DeviceArray shape/dtype semantics, memory lifetime and
host<->device transfer operations. Ported from the Numba-CUDA test-coverage
effort (NVIDIA/numba-cuda#515) and adapted to the Numba-CUDA-MLIR programming
model.
"""
...
class TestDeviceArrayManipulation(NumbaCUDATestCase):
    """Tests for DeviceArray memory behaviour and shape semantics."""
    ...
    @pytest.mark.xfail(reason="Boolean mask indexing is not supported on CUDA device arrays")
