...
from numba_cuda_mlir.numba_cuda.cudadrv.driver import CudaAPIError, driver
...
class TestInit(NumbaCUDATestCase):
    def _test_init_failure(self, target, expected):
        ...
        if not success:
            ...
            if msg is None:
                self.fail("CUDA error message missing after CudaSupportError")
            self.assertIn(expected, msg)
    ...
    def _test_cuda_disabled(self, target):
        ...
        try:
            ...
            self._test_init_failure(target, expected)
        finally:
