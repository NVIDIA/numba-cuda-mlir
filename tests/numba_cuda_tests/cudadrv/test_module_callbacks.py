...
from cusimt.descriptor import _LoadedModule
...
def is_valid_module_object(obj):
    """Check if obj is a valid module object (ObjectCode or _LoadedModule)."""
...
class TestModuleCallbacksAPICompleteness(NumbaCUDATestCase):
    def test_api(self):
        ...
        for setup, teardown in api_combo:
            setup_name = setup.__name__ if setup else "None"
            teardown_name = teardown.__name__ if teardown else "None"
            with self.subTest(setup=setup_name, teardown=teardown_name):
                lib = CUSource("", setup_callback=setup, teardown_callback=teardown)
