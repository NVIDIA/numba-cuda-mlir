...
from numba_cuda_mlir.numba_cuda.core import postproc, rewrites, ir_utils
...
class TestBranchPruneBase(NumbaCUDATestCase):
    """
...
class TestBranchPrune(TestBranchPruneBase):
    ...
    def test_single_if_else_two_const_val(self):
        ...
        self.assert_prune(
            impl,
            (types.IntegerLiteral(100),) * 2 + (types.Array(types.float64, 1, "C"),),
            [None],
            100,
            100,
        )
        self.assert_prune(
            impl,
            (types.NoneType("none"),) * 2 + (types.Array(types.float64, 1, "C"),),
            [False],
            None,
            None,
        )
