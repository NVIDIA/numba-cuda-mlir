...
def compile_scalar_func(pyfunc, argtypes, restype):
    ...
    device_func = cusimt.jit(restype(*argtypes), device=True)(pyfunc)
    ...
    kernel = cusimt.jit(tuple(kernel_types))(kernel_func)
...
class BaseComplexTest(NumbaCUDATestCase):
...
class TestComplex(BaseComplexTest):
    def check_real_image(self, pyfunc):
        ...
        self.run_unary(
            pyfunc,
            [tp.underlying_float(tp) for tp in (types.complex64, types.complex128)],
            values,
        )
...
class TestAtomicOnComplexComponents(NumbaCUDATestCase):
