...
from cusimt.numba_cuda.types import complex64, int32, float64
...
class TestCudaConstantMemory(NumbaCUDATestCase):
    ...
    def test_const_array(self):
        sig = (float64[:],)
        jcuconst = cusimt.jit(sig)(cuconst)
    ...
    def test_const_empty(self):
        jcuconstEmpty = cusimt.jit("void(int64[:])")(cuconstEmpty)
        A = np.full(1, fill_value=-1, dtype=np.int64)
        jcuconstEmpty[1, 1](A)
        self.assertTrue(np.all(A == 0))

    def test_const_align(self):
        jcuconstAlign = cusimt.jit("void(float64[:])")(cuconstAlign)
    ...
    def test_const_array_2d(self):
        sig = (int32[:, :],)
        jcuconst2d = cusimt.jit(sig)(cuconst2d)
    ...
    def test_const_array_3d(self):
        sig = (complex64[:, :, :],)
        jcuconst3d = cusimt.jit(sig)(cuconst3d)
    ...
    def test_const_record_empty(self):
        jcuconstRecEmpty = cusimt.jit("void(int64[:])")(cuconstRecEmpty)
        A = np.full(1, fill_value=-1, dtype=np.int64)
    ...
    def test_const_record(self):
        ...
        jcuconst = cusimt.jit(cuconstRec).specialize(A, B)
    ...
    def test_const_record_align(self):
        ...
        jcuconst = cusimt.jit(cuconstRecAlign).specialize(A, B, C, D, E)
