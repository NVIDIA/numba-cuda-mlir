...
class TestCudaDebugInfo(NumbaCUDATestCase):
    """
    ...
    def test_no_debuginfo_in_asm(self):
        @cusimt.jit(debug=False, opt=False)
        def foo(x):
            x[0] = 1
    ...
    def test_debuginfo_in_asm(self):
        @cusimt.jit(debug=True, opt=False)
        def foo(x):
            x[0] = 1
    ...
    def test_issue_5835(self):
        ...
        @cusimt.jit((types.int32[::1],), debug=True, opt=False)
        def f(x):
            x[0] = 0
    ...
    def test_issue_9888(self):
        ...
        @cusimt.jit(sig, debug=True, opt=False)
        def f(cond):
            if cond:
                x = 1  # noqa: F841
            else:
    ...
    def test_bool_type(self):
        ...
        @cusimt.jit("void(int32, int32)", debug=True, opt=False)
        def f(x, y):
            z = x == y  # noqa: F841
    ...
    def test_debug_function_calls_internal_impl(self):
        ...
        @cusimt.jit((types.int32[:], types.int32[:]), debug=True, opt=False)
        def f(inp, outp):
    ...
    def test_debug_function_calls_device_function(self):
        ...
        @cusimt.jit(device=True, debug=True, opt=0)
        def threadid():
            return cuda.blockDim.x * cuda.blockIdx.x + cuda.threadIdx.x

        @cusimt.jit((types.int32[:],), debug=True, opt=0)
        def kernel(arr):
            i = cuda.grid(1)
            if i < len(arr):
                arr[i] = threadid()
    ...
    def _test_chained_device_function(self, kernel_debug, f1_debug, f2_debug):
        @cusimt.jit(device=True, debug=f2_debug, opt=False)
        def f2(x):
            return x + 1

        @cusimt.jit(device=True, debug=f1_debug, opt=False)
        def f1(x, y):
            return x - f2(y)

        @cusimt.jit((types.int32, types.int32), debug=kernel_debug, opt=False)
        def kernel(x, y):
            f1(x, y)
    ...
    def _test_chained_device_function_two_calls(self, kernel_debug, f1_debug, f2_debug):
        ...
        @cusimt.jit(debug=kernel_debug, opt=False)
        def kernel(x, y):
            f1(x, y)
            f2(x)
    ...
    def test_chained_device_three_functions(self):
        ...
        def three_device_fns(kernel_debug, leaf_debug):
            @cusimt.jit(device=True, debug=leaf_debug, opt=False)
            def f3(x):
                return x * x
    ...
    def test_kernel_args_names(self):
        ...
        @cusimt.jit("void(int32)", debug=True, opt=False)
        def f(x):
            z = x  # noqa: F841
    ...
    def test_llvm_dbg_value_range(self):
        ...
        @cusimt.jit("void(int64,)", debug=True, opt=False)
        def foo(x):
            """
    ...
    def test_union_debug(self):
        @cusimt.jit("void(u8, int64[::1])", debug=True, opt=False)
    ...
    def test_poly_variant_part(self):
        ...
        @cusimt.jit("void()", debug=True, opt=False)
        def f():
            foo = 100  # noqa: F841
            foo = 3.14  # noqa: F841
            foo = True  # noqa: F841
    ...
    def test_missing_source(self):
        ...
        foo = cusimt.jit(debug=True, opt=False)(l["foo"])
    ...
    def test_linecache_source(self):
        ...
        try:
            ...
            debug_warnings = [x for x in w if x.category == NumbaDebugInfoWarning]
            self.assertEqual(
                len(debug_warnings),
                0,
    ...
    def test_DILocation_versioned_variables(self):
        ...
        foo_source_lines, foo_source_lineno = inspect.getsourcelines(foo.py_func)
        ...
        debug_info_lines = list(filter(lambda x: debuginfo_equals.search(x), lines))

        function_start_regex = re.compile(r"define void @.+foo")
        function_start_lines = list(
            filter(lambda x: function_start_regex.search(x[1]), enumerate(lines))
        )
        function_end_lines = list(filter(lambda x: x[1] == "}", enumerate(lines)))
        foo_ir_lines = lines[function_start_lines[0][0] : function_end_lines[0][0]]
        ...
        store_1_dbginfos = list(
            filter(lambda x: store_1_dbginfo_node + " = " in x, debug_info_lines)
        )
        store_5_dbginfos = list(
            filter(lambda x: store_5_dbginfo_node + " = " in x, debug_info_lines)
        )
        ...
        LineNumbers = namedtuple("LineNumbers", ["cond_branch", "store_5", "store_1"])
    ...
    def test_debuginfo_asm(self):
        ...
        foo_debug = cusimt.jit(debug=True, opt=False)(foo)
        foo_debug[1, 1]()
        ...
        foo_nodebug = cusimt.jit(debug=False)(foo)
        foo_nodebug[1, 1]()
    ...
    def test_DISubprogram_def_line_number(self):
        ...
        for kernel in kernels:
            source_lines, start_lineno = inspect.getsourcelines(kernel.py_func)
    ...
    def test_arg_load_has_dbg_location(self):
        ...
        @cusimt.jit("void(float32[:], int64)", debug=True, opt=False)
        def foo(arr, n):
            for j in range(n):
                arr[0] += j
            for j in range(n):
