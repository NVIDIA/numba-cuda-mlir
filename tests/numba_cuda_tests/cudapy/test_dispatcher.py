...
class TestDispatcherSpecialization(NumbaCUDATestCase):
...
class TestDispatcher(NumbaCUDATestCase):
    ...
    def test_coerce_input_types(self):
        ...
        c_add = cusimt.jit("(i4[::1], i4, i4)")(add_kernel)
        r = np.zeros(1, dtype=np.int32)

        c_add[1, 1](r, 123, 456)
        self.assertPreciseEqual(r[0], add(123, 456))

    @pytest.mark.xfail(True, reason="Numba-CUDA legacy reasons")
    ...
    @pytest.mark.skip(reason="Flaky test - some multithread issue")
    def test_lock(self):
        """
    ...
    @pytest.mark.xfail(True, reason="Unsafe conversions of arguments not handled")
    def test_explicit_signatures_unsafe(self):
        ...
        f = cusimt.jit("(int64[::1], int64, int64)")(add_kernel)
        r = np.zeros(1, dtype=np.int64)
    ...
    def add_device_usecase(self, sigs):
        ...
        add_device = cusimt.jit(sigs, device=True)(add)
    ...
    @pytest.mark.xfail(True, reason="Exception not raised")
...
class TestDispatcherKernelProperties(NumbaCUDATestCase):
    ...
    def test_get_regs_per_thread_specialized(self):
        @cusimt.jit(void(float32[::1], int64))
        def pi_sin_array(x, n):
            i = cuda.grid(1)
            if i < n:
                x[i] = 3.14 * math.sin(x[i])
...
class TestLaunchBounds(NumbaCUDATestCase):
    def _test_launch_bounds_common(self, launch_bounds):
        @cusimt.jit(launch_bounds=launch_bounds)
        def f():
            pass
    ...
    def test_too_many_launch_bounds(self):
        ...
        with self.assertRaisesRegex(ValueError, "Got 4 launch bounds:"):
            cusimt.jit("void()", launch_bounds=launch_bounds)(lambda: None)
    ...
    def test_shared_memory_carveout_invalid_values(self):
        ...
        for carveout, exc_type, msg_pattern in test_cases:
            ...
            with self.assertRaisesRegex(exc_type, msg_pattern):
                ...
                @cusimt.jit(shared_memory_carveout=carveout)
                def add_one(x):
                    i = cuda.grid(1)
                    if i < len(x):
                        x[i] = i + 1
            ...
            with self.assertRaisesRegex(exc_type, msg_pattern):
                ...
                @cusimt.jit("void(int32[:])", shared_memory_carveout=carveout)
                def add_one_sig(x):
                    i = cuda.grid(1)
                    if i < len(x):
                        x[i] = i + 1
