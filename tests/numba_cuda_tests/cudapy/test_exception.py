...
class TestException(NumbaCUDATestCase):
    @pytest.mark.xfail(True, reason="Raises unexpected IndexError")
    def test_exception(self):
        ...
        unsafe_foo = cusimt.jit(foo)
        safe_foo = cusimt.jit(debug=True, opt=False)(foo)
    ...
    def case_raise_causing_warp_diverge(self, with_debug_mode):
        ...
        @cusimt.jit(debug=with_debug_mode, opt=with_opt_mode)
        def problematic(x, y):
    ...
    @pytest.mark.xfail(True, reason="Expected exception not raised")
    ...
    @pytest.mark.xfail(True, reason="Raising in device functions does not work")
