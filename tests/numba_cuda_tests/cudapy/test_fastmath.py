...
class TestFastMathOption(NumbaCUDATestCase):
    def _test_fast_math_common(self, pyfunc, sig, device, criterion):
        # Test jit code path
        fastver = cusimt.jit(sig, device=device, fastmath=True)(pyfunc)
        precver = cusimt.jit(sig, device=device)(pyfunc)
    ...
    def test_divf_exception(self):
        ...
        fastver = cusimt.jit(sig, fastmath=True, debug=True, opt=False)(f10)
        precver = cusimt.jit(sig, debug=True, opt=False)(f10)
        nelem = 10
    ...
    def test_device_fastmath_propagation(self):
        ...
        # an xfail.
        @cusimt.jit("float32(float32, float32)", device=True)
        def foo(a, b):
            return a / b
        ...
        fastver = cusimt.jit(sig, fastmath=True)(bar)
        precver = cusimt.jit(sig)(bar)
