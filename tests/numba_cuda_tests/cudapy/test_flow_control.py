...
class TestFlowControl(NumbaCUDATestCase):
    def run_test(
        self,
        pyfunc,
        x_operands,
...
class TestCFGraph(NumbaCUDATestCase):
    """
    ...
    def test_topo_sort(self):
        def check_topo_sort(nodes, expected):
            ...
            np.random.RandomState(42).shuffle(nodes)
            self.assertIn(list(g.topo_sort(nodes)), expected)
            self.assertIn(list(g.topo_sort(nodes, reverse=True))[::-1], expected)
...
class TestRealCodeDomFront(NumbaCUDATestCase):
