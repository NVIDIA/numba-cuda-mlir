...
from cusimt.numba_cuda.types import float32, float64, int32, void
...
class TestCudaFrexpLdexp(NumbaCUDATestCase):
    def template_test_frexp(self, nptype, nbtype):
        compiled = cusimt.jit(void(nbtype[:], int32[:], nbtype))(simple_frexp)
        arg = 3.1415
    ...
    def template_test_ldexp(self, nptype, nbtype):
        compiled = cusimt.jit(void(nbtype[:], nbtype, int32))(simple_ldexp)
        arg = 0.785375
        exp = 2
        aryx = np.zeros(1, dtype=nptype)
