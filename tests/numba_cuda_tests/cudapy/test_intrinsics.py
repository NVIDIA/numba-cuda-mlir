...
class TestCudaIntrinsic(NumbaCUDATestCase):
    def setUp(self):
        super().setUp()
        np.random.seed(0)

    def test_simple_threadidx(self):
        compiled = cusimt.jit("void(int32[:])")(simple_threadidx)
        ary = np.ones(1, dtype=np.int32)
        compiled[1, 1](ary)
        self.assertTrue(ary[0] == 0)

    def test_fill_threadidx(self):
        compiled = cusimt.jit("void(int32[:])")(fill_threadidx)
        N = 10
    ...
    def test_fill3d_threadidx(self):
        ...
        def c_contigous():
            compiled = cusimt.jit("void(int32[:,:,::1])")(fill3d_threadidx)
    ...
    def test_nonliteral_grid_error(self):
        with self.assertRaisesRegex(TypingError, "RequireLiteralValue"):
            cusimt.jit("void(int32)")(nonliteral_grid)

    @pytest.mark.xfail(True, reason="ICE")
    def test_nonliteral_gridsize_error(self):
        with self.assertRaisesRegex(TypingError, "RequireLiteralValue"):
            cusimt.jit("void(int32)")(nonliteral_gridsize)

    def test_simple_grid1d(self):
        compiled = cusimt.jit("void(int32[::1])")(simple_grid1d)
        ntid, nctaid = 3, 7
        nelem = ntid * nctaid
    ...
    def test_simple_grid2d(self):
        compiled = cusimt.jit("void(int32[:,::1])")(simple_grid2d)
        ntid = (4, 3)
        nctaid = (5, 6)
    ...
    def test_simple_gridsize1d(self):
        compiled = cusimt.jit("void(int32[::1])")(simple_gridsize1d)
        ntid, nctaid = 3, 7
    ...
    def test_selp(self):
        ...
        cu_branching_with_ifs = cusimt.jit(sig)(branching_with_ifs)
        cu_branching_with_selps = cusimt.jit(sig)(branching_with_selps)
    ...
    def test_simple_gridsize2d(self):
        compiled = cusimt.jit("void(int32[::1])")(simple_gridsize2d)
        ntid = (4, 3)
        nctaid = (5, 6)
    ...
    def test_intrinsic_forloop_step(self):
        compiled = cusimt.jit("void(int32[:,::1])")(intrinsic_forloop_step)
        ntid = (4, 3)
        nctaid = (5, 6)
        shape = (ntid[0] * nctaid[0], ntid[1] * nctaid[1])
        ary = np.empty(shape, dtype=np.int32)
    ...
    def test_hadd_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hadd_scalar)
        ary = np.zeros(1, dtype=np.float16)
    ...
    def test_hadd_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)", lto=True)(simple_hadd_scalar)
        args = (f2[:], f2, f2)
        ptx = compiled.inspect_lto_ptx(args)
        self.assertIn("add.f16", ptx)

    def test_hfma(self):
        compiled = cusimt.jit("void(f2[:], f2[:], f2[:], f2[:])")(simple_hfma)
        ary = np.zeros(1, dtype=np.float16)
    ...
    def test_hfma_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2, f2, f2)")(simple_hfma_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(2.0)
        arg2 = np.float16(3.0)
        arg3 = np.float16(4.0)
    ...
    def test_hfma_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2, f2, f2)", lto=True)(simple_hfma_scalar)
        args = (f2[:], f2, f2, f2)
        ptx = compiled.inspect_lto_ptx(args)
        self.assertIn("fma.rn.f16", ptx)

    def test_hsub(self):
        compiled = cusimt.jit("void(f2[:], f2[:], f2[:])")(simple_hsub)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.array([3.0], dtype=np.float16)
        arg2 = np.array([4.0], dtype=np.float16)
        compiled[1, 1](ary, arg1, arg2)
    ...
    def test_hsub_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hsub_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.1415926)
    ...
    def test_hsub_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)", lto=True)(simple_hsub_scalar)
        args = (f2[:], f2, f2)
        ptx = compiled.inspect_lto_ptx(args)
        self.assertIn("sub.f16", ptx)

    def test_hmul(self):
        compiled = cusimt.jit(simple_hmul)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.array([3.0], dtype=np.float16)
        arg2 = np.array([4.0], dtype=np.float16)
        compiled[1, 1](ary, arg1, arg2)
    ...
    def test_hmul_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hmul_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.1415926)
        arg2 = np.float16(1.57)
        compiled[1, 1](ary, arg1, arg2)
    ...
    def test_hmul_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)", lto=True)(simple_hmul_scalar)
        args = (f2[:], f2, f2)
        ptx = compiled.inspect_lto_ptx(args)
        self.assertIn("mul.f16", ptx)

    def test_hdiv_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hdiv_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.1415926)
        arg2 = np.float16(1.57)
    ...
    def test_hdiv(self):
        compiled = cusimt.jit("void(f2[:], f2[:], f2[:])")(simple_hdiv_kernel)
    ...
    def test_hneg(self):
        compiled = cusimt.jit("void(f2[:], f2[:])")(simple_hneg)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.array([3.0], dtype=np.float16)
    ...
    def test_hneg_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2)")(simple_hneg_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.1415926)
        compiled[1, 1](ary, arg1)
        ref = -arg1
    ...
    def test_hneg_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2)", lto=True)(simple_hneg_scalar)
        args = (f2[:], f2)
        ptx = compiled.inspect_lto_ptx(args)
        self.assertIn("neg.f16", ptx)

    def test_habs(self):
        compiled = cusimt.jit(simple_habs)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.array([-3.0], dtype=np.float16)
        compiled[1, 1](ary, arg1)
    ...
    def test_habs_scalar(self):
        compiled = cusimt.jit("void(f2[:], f2)")(simple_habs_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(-3.1415926)
        compiled[1, 1](ary, arg1)
        ref = abs(arg1)
    ...
    def test_habs_ptx(self):
        compiled = cusimt.jit("void(f2[:], f2)", lto=True)(simple_habs_scalar)
        args = (f2[:], f2)
        ptx = compiled.inspect_lto_ptx(args)
    ...
    def test_fp16_intrinsics_common(self):
        ...
        for kernel, fn in zip(kernels, expected_functions):
            kernel = cusimt.jit("void(f2[:], f2[:])")(kernel)
            kernel[1, N](r, x)
    ...
    def test_fp16_comparison(self):
        ...
        for fn, op in zip(fns, ops):
            kernel = cusimt.jit("void(b1[:], f2, f2)")(fn)
    ...
    def test_multiple_float16_comparisons(self):
        ...
        for fn in functions:
            compiled = cusimt.jit("void(b1[:], f2, f2, f2)")(fn)
    ...
    def test_hmax(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hmax_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.0)
        arg2 = np.float16(4.0)
        compiled[1, 1](ary, arg1, arg2)
    ...
    def test_hmin(self):
        compiled = cusimt.jit("void(f2[:], f2, f2)")(simple_hmin_scalar)
        ary = np.zeros(1, dtype=np.float16)
        arg1 = np.float16(3.0)
        arg2 = np.float16(4.0)
        compiled[1, 1](ary, arg1, arg2)
    ...
    def test_cbrt_f32(self):
        compiled = cusimt.jit("void(float32[:], float32)")(simple_cbrt)
        ary = np.zeros(1, dtype=np.float32)
        cbrt_arg = 2.0
    ...
    def test_cbrt_f64(self):
        compiled = cusimt.jit("void(float64[:], float64)")(simple_cbrt)
        ary = np.zeros(1, dtype=np.float64)
        cbrt_arg = 6.0
        compiled[1, 1](ary, cbrt_arg)
        np.testing.assert_allclose(ary[0], cbrt_arg ** (1 / 3))
    ...
    def test_clz_i4(self):
        compiled = cusimt.jit("void(int32[:], int32)")(simple_clz)
        ary = np.zeros(1, dtype=np.int32)
    ...
    def test_clz_u4(self):
        ...
        """
        compiled = cusimt.jit("void(int32[:], uint32)")(simple_clz)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary, 0x00100000)
        self.assertEqual(ary[0], 11)
    ...
    def test_clz_i8(self):
        compiled = cusimt.jit("void(int32[:], int64)")(simple_clz)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary, 0x000000000010000)
        self.assertEqual(ary[0], 47)

    def test_ffs_i4(self):
        compiled = cusimt.jit("void(int32[:], int32)")(simple_ffs)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary, 0x00100000)
    ...
    def test_ffs_u4(self):
        compiled = cusimt.jit("void(int32[:], uint32)")(simple_ffs)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary, 0x00100000)
        self.assertEqual(ary[0], 21)
        compiled[1, 1](ary, 0x80000000)
    ...
    def test_ffs_i8(self):
        compiled = cusimt.jit("void(int32[:], int64)")(simple_ffs)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary, 0x000000000010000)
    ...
    def test_simple_laneid(self):
        compiled = cusimt.jit("void(int32[:])")(simple_laneid)
        count = 2
    ...
    def test_simple_warpsize(self):
        compiled = cusimt.jit("void(int32[:])")(simple_warpsize)
        ary = np.zeros(1, dtype=np.int32)
        compiled[1, 1](ary)
        self.assertEqual(ary[0], 32, "CUDA semantics")
    ...
    def test_round_f4(self):
        compiled = cusimt.jit("void(int64[:], float32)")(simple_round)
    ...
    def test_round_f8(self):
        compiled = cusimt.jit("void(int64[:], float64)")(simple_round)
        ary = np.zeros(1, dtype=np.int64)
    ...
    def test_round_to_f4(self):
        compiled = cusimt.jit("void(float32[:], float32, int32)")(simple_round_to)
        ary = np.zeros(1, dtype=np.float32)
        np.random.seed(123)
    ...
    def test_round_to_f8(self):
        compiled = cusimt.jit("void(float64[:], float64, int32)")(simple_round_to)
        ary = np.zeros(1, dtype=np.float64)
        np.random.seed(123)
