...
class TestCudaLineInfo(NumbaCUDATestCase):
    ...
    def test_debug_and_lineinfo_warning(self):
        with pytest.warns(
            ...
            # occurring as well
            @cusimt.jit(debug=True, lineinfo=True, opt=False)
            def f():
                pass
    ...
    def test_lineinfo_with_compile_internal(self):
        ...
        @cusimt.jit("void(complex128[::1], complex128[::1])", lineinfo=True)
