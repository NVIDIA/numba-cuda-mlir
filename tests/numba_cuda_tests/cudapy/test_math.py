...
class TestCudaMath(NumbaCUDATestCase):
    ...
    def unary_template(self, func, npfunc, npdtype, nprestype, start, stop):
        ...
        cfunc = cusimt.jit((arytype, restype))(func)
        cfunc[1, nelem](A, B)
    ...
    def unary_bool_special_values(self, func, npfunc, npdtype, npmtype):
        ...
        cfunc = cusimt.jit((npmtype[::1], int32[::1]))(func)
    ...
    def unary_bool_template(self, func, npfunc, npdtype, npmtype, start, stop):
        ...
        iarytype = npmtype[::1]
        oarytype = int32[::1]
        cfunc = cusimt.jit((iarytype, oarytype))(func)
        cfunc[1, nelem](A, B)
    ...
    def binary_template(self, func, npfunc, npdtype, nprestype, start, stop):
        ...
        arytype = numpy_support.from_dtype(npdtype)[::1]
        restype = numpy_support.from_dtype(nprestype)[::1]
        cfunc = cusimt.jit((arytype, arytype, restype))(func)
        cfunc[1, nelem](A, A, B)
    ...
    def test_math_remainder_0_0(self):
        @cusimt.jit(void(float64[::1], int64, int64))
        def test_0_0(r, x, y):
    ...
    def pow_template_int32(self, npdtype):
        ...
        C = np.empty_like(A)
        arytype = numpy_support.from_dtype(npdtype)[::1]
        cfunc = cusimt.jit((arytype, int32[::1], arytype))(math_pow)
        cfunc[1, nelem](A, B, C)
    ...
    def test_math_modf(self):
        def modf_template_nan(dtype, arytype):
            ...
            B = np.zeros_like(A)
            C = np.zeros_like(A)
            cfunc = cusimt.jit((arytype, arytype, arytype))(math_modf)
            cfunc[1, len(A)](A, B, C)
