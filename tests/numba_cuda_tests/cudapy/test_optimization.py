...
class TestOptimization(NumbaCUDATestCase):
    def test_eager_opt(self):
        ...
        sig = (float64[::1],)
        kernel = cusimt.jit(sig)(kernel_func)
    ...
    def test_eager_noopt(self):
        ...
        sig = (float64[::1],)
        kernel = cusimt.jit(sig, opt=False)(kernel_func)
        ptx = kernel.inspect_asm()
    ...
    def test_lazy_noopt(self):
        # Optimization disabled
        kernel = cusimt.jit(opt=False)(kernel_func)
        x = np.zeros(1, dtype=np.float64)
        kernel[1, 1](x)
    ...
    def test_device_opt(self):
        ...
        device = cusimt.jit(sig, device=True)(device_func)
    ...
    def test_device_noopt(self):
        # Optimization disabled
        sig = (float64, float64, float64)
        device = cusimt.jit(sig, device=True, opt=False)(device_func)
        ptx = device.inspect_asm(sig)
