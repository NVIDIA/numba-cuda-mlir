...
class TestRecordDtype(NumbaCUDATestCase):
...
class TestNestedArrays(NumbaCUDATestCase):
    ...
    def get_cfunc(self, pyfunc, retty):
        ...
        inner = cusimt.jit(device=True)(pyfunc)
    ...
    @pytest.mark.xfail(True, reason="Cannot return arrays from device functions")
