...
from cusimt.numba_cuda.types import int32, float32
...
class TestCudaSync(NumbaCUDATestCase):
    def _test_useless(self, kernel):
        compiled = cusimt.jit("void(int32[::1])")(kernel)
        nelem = 10
    ...
    def test_coop_syncwarp(self):
        ...
        compiled = cusimt.jit("void(int32[::1])")(coop_syncwarp)
    ...
    def test_simple_smem(self):
        compiled = cusimt.jit("void(int32[::1])")(simple_smem)
        nelem = 100
        ary = np.empty(nelem, dtype=np.int32)
        compiled[1, nelem](ary)
    ...
    def test_coop_smem2d(self):
        compiled = cusimt.jit("void(float32[:,::1])")(coop_smem2d)
        shape = 10, 20
    ...
    def test_dyn_shared_memory(self):
        compiled = cusimt.jit("void(float32[::1])")(dyn_shared_memory)
        shape = 50
        ary = np.empty(shape, dtype=np.float32)
    ...
    def test_threadfence_codegen(self):
        ...
        sig = (int32[:],)
        compiled = cusimt.jit(sig)(use_threadfence)
    ...
    def test_threadfence_block_codegen(self):
        # Does not test runtime behavior, just the code generation.
        sig = (int32[:],)
        compiled = cusimt.jit(sig)(use_threadfence_block)
        ary = np.zeros(10, dtype=np.int32)
        compiled[1, 1](ary)
        self.assertEqual(123 + 321, ary[0])
    ...
    def test_threadfence_system_codegen(self):
        # Does not test runtime behavior, just the code generation.
        sig = (int32[:],)
        compiled = cusimt.jit(sig)(use_threadfence_system)
        ary = np.zeros(10, dtype=np.int32)
        compiled[1, 1](ary)
        self.assertEqual(123 + 321, ary[0])
    ...
    def _test_syncthreads_count(self, in_dtype):
        compiled = cusimt.jit(use_syncthreads_count)
    ...
    def _test_syncthreads_and(self, in_dtype):
        compiled = cusimt.jit(use_syncthreads_and)
        nelem = 100
    ...
    def _test_syncthreads_or(self, in_dtype):
        compiled = cusimt.jit(use_syncthreads_or)
        nelem = 100
