...
class TestCudaWarpOperations(NumbaCUDATestCase):
    def test_useful_syncwarp(self):
        compiled = cusimt.jit("void(int32[:])")(useful_syncwarp)
        nelem = 32
    ...
    def test_shfl_sync_idx(self):
        compiled = cusimt.jit("void(int32[:], int32)")(use_shfl_sync_idx)
        nelem = 32
        idx = 4
        ary = np.empty(nelem, dtype=np.int32)
    ...
    def test_shfl_sync_up(self):
        compiled = cusimt.jit("void(int32[:], int32)")(use_shfl_sync_up)
        nelem = 32
        delta = 4
        ary = np.empty(nelem, dtype=np.int32)
    ...
    def test_shfl_sync_down(self):
        compiled = cusimt.jit("void(int32[:], int32)")(use_shfl_sync_down)
        nelem = 32
        delta = 4
        ary = np.empty(nelem, dtype=np.int32)
        exp = np.arange(nelem, dtype=np.int32)
    ...
    def test_shfl_sync_xor(self):
        compiled = cusimt.jit("void(int32[:], int32)")(use_shfl_sync_xor)
        nelem = 32
        xor = 16
        ary = np.empty(nelem, dtype=np.int32)
    ...
    def test_shfl_sync_const_mode_val(self):
        ...
        for func, value in subtest:
            compiled = cusimt.jit("void(int32[:], int32)")(func)
            nelem = 32
            ary = np.empty(nelem, dtype=np.int32)
    ...
    def test_shfl_sync_types(self):
        ...
        for typ, val in zip(types, values):
            compiled = cusimt.jit((typ[:], typ))(use_shfl_sync_with_val)
            nelem = 32
    ...
    def test_vote_sync_type_validation(self):
        ...
        for sig, expected_msg in invalid_cases:
            with self.assertRaisesRegex(errors.TypingError, expected_msg):
                cusimt.jit(sig)(use_vote_sync_all_with_mask)
        ...
        for sig, mask_dtype, pred_dtype in valid_cases:
            ...
            compiled = cusimt.jit(sig)(use_vote_sync_all_with_mask)
    ...
    def test_vote_sync_all(self):
        compiled = cusimt.jit("void(int32[:], int32[:])")(use_vote_sync_all)
        nelem = 32
    ...
    def test_vote_sync_any(self):
        compiled = cusimt.jit("void(int32[:], int32[:])")(use_vote_sync_any)
        nelem = 32
    ...
    def test_vote_sync_eq(self):
        compiled = cusimt.jit("void(int32[:], int32[:])")(use_vote_sync_eq)
        nelem = 32
        ary_in = np.zeros(nelem, dtype=np.int32)
        ary_out = np.empty(nelem, dtype=np.int32)
        compiled[1, nelem](ary_in, ary_out)
    ...
    def test_vote_sync_ballot(self):
        compiled = cusimt.jit("void(uint32[:])")(use_vote_sync_ballot)
        nelem = 32
    ...
    def test_match_any_sync(self):
        compiled = cusimt.jit("void(int32[:], int32[:])")(use_match_any_sync)
        nelem = 10
    ...
    def test_match_all_sync(self):
        compiled = cusimt.jit("void(int32[:], int32[:])")(use_match_all_sync)
        nelem = 10
        ary_in = np.zeros(nelem, dtype=np.int32)
        ary_out = np.empty(nelem, dtype=np.int32)
        compiled[1, nelem](ary_in, ary_out)
    ...
    def test_independent_scheduling(self):
        compiled = cusimt.jit("void(uint32[:])")(use_independent_scheduling)
