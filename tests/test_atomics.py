...
from cuda.simt import uint32, uint64, float32, float64, int32
...
def atomic_compare_and_swap(res, old, ary, fill_val):
    gid = cuda.grid(1)
    if gid < res.size:
        old[gid] = cuda.atomic.compare_and_swap(res[gid : res.size], fill_val, ary[gid])
...
class TestCudaAtomics(unittest.TestCase):
    ...
    def test_atomic_add(self):
        ...
        assert np.all(ary_wrap == gold)

    def test_atomic_add2(self):
        orig = np.random.randint(0, 32, size=32).astype(np.uint32).reshape(4, 8)
        ary_wrap = cuda.to_device(orig)
        ary = cuda.to_device(orig)
        ...
        assert np.all(ary.copy_to_host() == orig + 1)
        assert np.all(ary_wrap.copy_to_host() == orig + 1)
    ...
    def test_atomic_add3(self):
        ...
        cuda_atomic_add3[1, (4, 8)](dary)

        assert np.all(dary.copy_to_host() == orig + 1)

    def test_atomic_add_float(self):
        ary = np.random.randint(0, 32, size=32).astype(np.float32)
        orig = ary.copy().astype(np.intp)
        dary = cuda.to_device(ary)
        dary_wrap = cuda.to_device(orig.astype(np.float32))

        cuda_atomic_add_float = cuda.jit("void(float32[:])")(atomic_add_float)
        cuda_atomic_add_float[1, 32](dary)

        add_float_wrap = cuda.jit("void(float32[:])")(atomic_add_float_wrap)
        add_float_wrap[1, 32](dary_wrap)

        gold = np.zeros(32, dtype=np.uint32)
        for i in range(orig.size):
            gold[orig[i]] += 1.0

        assert np.all(dary.copy_to_host() == gold)
        assert np.all(dary_wrap.copy_to_host() == gold)
    ...
    def test_atomic_add_float_2(self):
        ...
        dary = cuda.to_device(ary)
        dary_wrap = cuda.to_device(ary_wrap)

        cuda_atomic_add2 = cuda.jit("void(float32[:,:])")(atomic_add_float_2)
        cuda_atomic_add2[1, (4, 8)](dary)

        cuda_func_wrap = cuda.jit("void(float32[:,:])")(atomic_add_float_2_wrap)
        cuda_func_wrap[1, (4, 8)](dary_wrap)

        assert np.all(dary.copy_to_host() == orig + 1)
        assert np.all(dary_wrap.copy_to_host() == orig + 1)
    ...
    def assertCorrectFloat64Atomics(self, kernel, shared=True):
        ...
        if tools.get_gpu_compute_capability() >= (6, 0):
            inst = "(red|atom)"
    ...
    def test_atomic_add_double(self):
        ...
        cuda_fn[1, 32, 0, 32 * 8](idx, ary)

        wrap_fn = cuda.jit("void(int64[:], float64[:])")(atomic_add_double_wrap)
        wrap_fn[1, 32, 0, 32 * 8](idx, ary_wrap)
    ...
    def test_atomic_add_double_global(self):
        ...
        cuda_func[1, 32, 0, 32 * 8](idx, ary)
        wrap_cuda_func[1, 32, 0, 32 * 8](idx, ary_wrap)
    ...
    def test_atomic_add_double_global_2(self):
        ...
        cuda_func[1, (4, 8), 0, 32 * 8](ary)
        wrap_cuda_func[1, (4, 8), 0, 32 * 8](ary_wrap)
    ...
    def test_atomic_sub(self):
        orig = np.random.randint(0, 32, size=32, dtype=np.int32)
        ary = cuda.to_device(orig)
        cuda_atomic_sub = cuda.jit("void(int32[:])")(atomic_sub)
        cuda_atomic_sub[1, 32, 0, 32 * 4](ary)

        gold = np.zeros(32, dtype=np.int32)
        for i in range(orig.size):
            gold[orig[i]] -= 1

        assert np.all(ary.copy_to_host() == gold)
    ...
    def test_atomic_sub_float(self):
        ...
        cuda_atomic_sub_float[1, 32, 0, 32 * 4](dary)
    ...
    def test_atomic_sub_double(self):
        idx = np.random.randint(0, 32, size=32, dtype=np.int64)
        didx = cuda.to_device(idx)
        ary = np.zeros(32, np.float64)
        dary = cuda.to_device(ary)
        cuda_func = cuda.jit("void(int64[:], float64[:])", dump=False)(atomic_sub_double)
        cuda_func[1, 32, 0, 32 * 8](didx, dary)

        gold = np.zeros(32, dtype=np.float64)
        for i in range(idx.size):
            gold[idx[i]] -= 1.0

        np.testing.assert_equal(dary.copy_to_host(), gold)
    ...
    def test_atomic_and2(self):
        ...
        assert np.all(ary == orig & rand_const)
    ...
    def test_atomic_exch(self):
        ...
        cuda_func = cuda.jit("void(uint32[:], uint32[:], uint32)", dump=True, print_after_all=True)(
            atomic_exch
        )
    ...
    def test_atomic_exch3(self):
        ...
        cuda_func = cuda.jit("void(uint64[:,:], uint64)", dump=True, print_after_all=True)(
            atomic_exch3
        )
        cuda_func[1, (4, 8)](ary, rand_const)
        np.testing.assert_equal(ary, rand_const)
    ...
    def _test_atomic_minmax_nan_val(self, func):
        cuda_func = cuda.jit("void(float64[:], float64[:,:])", dump=True)(func)
    ...
    @pytest.mark.xfail(
        reason="atomicrmw fmin/fmax (hardware atomic) replaces NaN in memory; "
        "preserving NaN would require a CAS loop with ~10x worse performance"
    )
