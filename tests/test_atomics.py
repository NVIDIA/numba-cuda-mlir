...
from numba_cuda_mlir.cuda import uint32, uint64, float32, float64, int32
...
class TestCudaAtomics(unittest.TestCase):
    ...
    def test_atomic_sub_double(self):
        ...
        ary = np.zeros(32, np.float64)
        dary = cuda.to_device(ary)
        cuda_func = cuda.jit("void(int64[:], float64[:])", dump=False)(
            atomic_sub_double
        )
    ...
    def test_atomic_exch(self):
        ...
        cuda_func = cuda.jit(
            "void(uint32[:], uint32[:], uint32)", dump=True, print_after_all=True
        )(atomic_exch)
    ...
    def test_atomic_exch3(self):
        ...
        cuda_func = cuda.jit(
            "void(uint64[:,:], uint64)", dump=True, print_after_all=True
        )(atomic_exch3)
        cuda_func[1, (4, 8)](ary, rand_const)
        np.testing.assert_equal(ary, rand_const)
