...
from cusimt import cuda, extending, types, testing
import numpy as np


def test_extending_intrinsic():
    def codegen(builder, target, args, kwargs):
        from cusimt._mlir.dialects import cf, arith
...
def test_extending_overload_with_lowering():
    from cusimt.extending import lowering_registry
...
def test_struct_field_model():
    ...
    from cusimt.models import UnicodeTypeModel

    with ir.Context(), ir.Location.unknown():
        from cusimt.descriptor import MLIRTargetContext, MLIRTypingContext

        tc = MLIRTypingContext()
        tc.refresh()
        ctx = MLIRTargetContext(tc, target="cusimt")
        ctx.refresh()
...
def test_aggregate_type_field_model():
    ...
    from cusimt._mlir import ir
    from cusimt.models import AggregateTypeModel
