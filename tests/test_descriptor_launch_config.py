...
def test_launch_config_key_validation():
    assert descriptor_mod._launch_config_key(
        {
            "grid": (2, 1, 1),
            "block": (32, 1, 1),
            "sharedmem": None,
    ...
    with pytest.raises(TypeError, match="block"):
        descriptor_mod._launch_config_key({"block": 32, "sharedmem": 0, "cluster": None})
...
def test_inspection_keys_keep_generic_overload_shape():
    ...
    assert ((types.int64,), launch_key) not in dispatcher._launch_config_overloads
    assert dispatcher.inspect_asm((launch_sig, {"block": (32, 1, 1)})) == "launch"
...
def test_compile_impl_launch_config_publishes_aliases_and_skips_disk_cache(monkeypatch):
    ...
    first_launch_config = {"block": (32, 1, 1), "sharedmem": 0, "cluster": None}
    ...
    with pytest.warns(
        ...
        second_launch_config = {"block": (64, 1, 1), "sharedmem": 0, "cluster": None}
