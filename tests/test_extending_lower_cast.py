...
"""Tests for cusimt.extending.lower_cast — the external API that allows
...
or explicit coercion), cusimt's lowering consults the cast registry before
falling back to the default ``mlir_convert`` path.
"""

import numpy as np

from cusimt import cuda, extending, types
from cusimt.extending import lower_cast, lowering_registry
...
def make_boxed_int(x):
    raise NotImplementedError("only callable inside a cusimt kernel")
