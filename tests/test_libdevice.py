...
from cuda.simt import types, compiler, libdevice, libdevicefuncs

from cuda.simt.libdevice import (
...
def use_frexp_module_attr(frac, exp, x):
    ...
    if i < len(x):
        fracr, expr = cs.libdevice.frexp(x[i])
        frac[i] = fracr
        exp[i] = expr
...
def use_sad_module_attr(r, x, y, z):
    ...
    if i < len(x):
        r[i] = cs.libdevice.sad(x[i], y[i], z[i])
...
@pytest.mark.parametrize(
    ...
    [
        (use_sincos_module_attr, "cs.libdevice.sincos"),
...
def test_sincos(kernel_func, import_style):
    ...
    arr = cs.to_device(np.arange(100, dtype=np.float64))
    sres = cs.to_device(np.zeros_like(arr))
    cres = cs.to_device(np.zeros_like(arr))

    cufunc = cs.jit(kernel_func)
...
@pytest.mark.parametrize(
    "kernel_func,import_style",
    [
        (use_frexp_module_attr, "cs.libdevice.frexp"),
...
def test_frexp(kernel_func, import_style):
    ...
    arr = cs.to_device(np.linspace(start=1.0, stop=10.0, num=100, dtype=np.float64))
    fracres = cs.to_device(np.zeros_like(arr))
    expres = cs.to_device(np.zeros(shape=arr.shape, dtype=np.int32))
...
@pytest.mark.parametrize(
    "kernel_func,import_style",
    [
        (use_sad_module_attr, "cs.libdevice.sad"),
...
def test_sad(kernel_func, import_style):
    ...
    x = cs.to_device(np.arange(0, 200, 2))
    y = cs.to_device(np.arange(50, 150))
    z = cs.to_device(np.arange(15, 115))
    r = cs.to_device(np.zeros_like(x))
