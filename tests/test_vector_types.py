...
from cusimt.testing import filecheck, filecheck_with_comments
...
def test_vector_load_store_1d():
    ...
    def kernel(arr_in, arr_out):
        i = cs.threadIdx.x * 4
        vec = cs.vector.load(arr_in, i, 4)
        cs.vector.store(arr_out, i, vec)
...
def test_vector_load_store_aligned():
    ...
    def kernel(arr_in, arr_out):
        ...
        if i < 8:
            ...
            alignment = 8
            vec = cs.vector.load(arr_in, idx, 4, alignment=alignment)
            cs.vector.store(arr_out, idx, vec, alignment)
...
def test_vector_load_store_2d_array():
    ...
    def kernel(arr_in, arr_out):
        row = cs.threadIdx.x
        vec = cs.vector.load(arr_in, (row, 0), 4)
        cs.vector.store(arr_out, (row, 0), vec)
...
def test_vector_2d_shape():
    ...
    def kernel(arr_in, arr_out):
        row = cs.threadIdx.x
        vec = cs.vector.load(arr_in, (row * 4, 0), (4, 4))
        cs.vector.store(arr_out, (row * 4, 0), vec)
...
def test_cuda_vector_float64x4_basic():
    ...
    @cs.jit
    def kernel(arr):
        v = cs.float64x4(1.0, 3.0, 5.0, 7.0)
        arr[0] = v.x
        arr[1] = v.y
        arr[2] = v.z
        arr[3] = v.w
...
def test_cuda_vector_float32x4_basic():
    ...
    @cs.jit
    def kernel(arr):
        v = cs.float32x4(2.0, 4.0, 6.0, 8.0)
        arr[0] = v.x
        arr[1] = v.y
        arr[2] = v.z
        arr[3] = v.w
...
@pytest.mark.parametrize(
    ...
    [
        (cs.float32x1, 1, np.float32),
        (cs.float32x2, 2, np.float32),
        (cs.float32x3, 3, np.float32),
        (cs.float32x4, 4, np.float32),
        (cs.float64x1, 1, np.float64),
        (cs.float64x2, 2, np.float64),
        (cs.float64x3, 3, np.float64),
        (cs.float64x4, 4, np.float64),
        (cs.int32x1, 1, np.int32),
        (cs.int32x2, 2, np.int32),
        (cs.int32x3, 3, np.int32),
        (cs.int32x4, 4, np.int32),
    ],
)
...
def test_cuda_vector_fancy_creation():
    ...
    def kernel(res):
        ...
        f2_1 = cs.float64x2(two, three)  # two scalars
        f2_2 = cs.float64x2(f1_1, three)  # vec1 + scalar
        f2_3 = cs.float64x2(two, f1_1)  # scalar + vec1
        f2_4 = cs.float64x2(f1_1, f1_1)  # vec1 + vec1
        f2_5 = cs.float64x2(f2_1)  # copy from vec2
        ...
        f3_1 = cs.float64x3(one, two, three)  # three scalars
        f3_2 = cs.float64x3(f2_1, one)  # vec2 + scalar
        f3_3 = cs.float64x3(one, f2_1)  # scalar + vec2
        f3_4 = cs.float64x3(f1_1, f1_1, f1_1)  # vec1 + vec1 + vec1
        ...
        f4_1 = cs.float64x4(one, two, three, four)  # four scalars
        f4_2 = cs.float64x4(f2_1, f2_1)  # vec2 + vec2
        f4_3 = cs.float64x4(f2_1, three, four)  # vec2 + scalar + scalar
        f4_4 = cs.float64x4(one, f2_1, four)  # scalar + vec2 + scalar
        f4_5 = cs.float64x4(one, two, f2_1)  # scalar + scalar + vec2
        f4_6 = cs.float64x4(f1_1, f1_1, f1_1, f1_1)  # four vec1s
